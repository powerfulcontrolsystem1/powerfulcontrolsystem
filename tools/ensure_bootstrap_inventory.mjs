...
const entries = [];
const sourceFunctionsByName = new Map();
for (const file of files) {
  const source = fs.readFileSync(file, "utf8");
  const relative = path.relative(repoRoot, file).replace(/\\/g, "/");
  const allFunctions = /func\s+(?:\([^)]*\)\s*)?([A-Za-z0-9_]+)\s*\(/g;
  for (const match of source.matchAll(allFunctions)) {
    sourceFunctionsByName.set(match[1], sha256(functionBody(source, match.index ?? 0)));
  }
...
const legacyManifestEntries = uniqueLegacyCatalogNames.map((name) => {
  const fingerprint = sourceFunctionsByName.get(name) ?? byName.get(name)?.bodyFingerprint;
  if (!fingerprint) throw new Error(`legacy schema catalog step ${name} has no source fingerprint`);
...
if (checkOnly) {
  ...
  if (normalizeLineEndings(current) !== rendered || normalizeLineEndings(currentLegacyManifest) !== renderedLegacyManifest) {
