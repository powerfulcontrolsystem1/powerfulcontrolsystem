// Adapted from platzhersh/pacman-canvas (CC0). See CC0.txt and ../creditos.html.
// PCS: native DOM, local assets, synthetic audio, serializable state, single external clock.
/*-------------------------------------------------------------------
...
export async function createPacman(target, sound) {
	/* ----- Global Variables ---------------------------------------- */
	var canvas;
	var context;
	var game;
	...
	const initialMap = await fetch('/juegos/pacman/map.json').then(r=>{if(!r.ok)throw Error('No se pudo cargar Pac-Man');return r.json()});
 const $ = () => ({get:()=>target,html(){return this},text(){return this},hide(){return this},show(){return this},fadeIn(){return this},fadeOut(){return this},slideToggle(){return this},toggle(){return this},focus(){return this},css:()=> 'none'});
 $.each=(items, fn)=>items.forEach((item,i)=>fn.call(item,i,item));
 const console={log(){},debug(){},warn(){},groupCollapsed(){},groupEnd(){},table(){},error(){}};
 let pendingLevel=null, frame=0;
	...
	function Game() {
		...
		this.nextLevel = function () {
			...
			if (this.level === FINAL_LEVEL) {
				...
				game.pause=true; pendingLevel=this.init(1).then(()=>{game.pause=false;pendingLevel=null});
			}
		};
		...
		this.drawHearts = function (count) {
			...
			for (var i = 0; i < count; i++) {
				html += " <img src='/juegos/pacman/img/heart.png'>";
			}
		...
		this.forceStartAnimationLoop = function () {
			...
			this.started = true;
			this.closeMessage();
			// The PCS room owns the only animation loop.
		}
		...
		this.loadMapConfig = async () => JSON.parse(JSON.stringify(initialMap));
		...
		this.init = async (state) => {
			...
			if (pinky === null || pinky === undefined) {
				pinky = new Ghost(GHOSTS.PINKY, 7, 5, '/juegos/pacman/img/pinky.svg', 2, 2);
				inky = new Ghost(GHOSTS.INKY, 8, 5, '/juegos/pacman/img/inky.svg', 13, 11);
				blinky = new Ghost(GHOSTS.BLINKY, 9, 5, '/juegos/pacman/img/blinky.svg', 13, 0);
				clyde = new Ghost(GHOSTS.CLYDE, 10, 5, '/juegos/pacman/img/clyde.svg', 2, 11);
			} else {
				pinky.reset();
				inky.reset();
				blinky.reset();
	...
	var Sound = {};
	Sound.play = kind => sound.effect(kind==='die'?'lose':kind==='waka'?'move':'power');
	...
	function Ghost(name, gridPosX, gridPosY, image, gridBaseX, gridBaseY) {
		...
		this.dazzleImg.src = '/juegos/pacman/img/dazzled.svg';
		this.dazzleImg2 = new Image();
		this.dazzleImg2.src = '/juegos/pacman/img/dazzled2.svg';
		this.deadImg = new Image();
		this.deadImg.src = '/juegos/pacman/img/dead.svg';
	...
	function animationLoop() {
		...
		if (game.dieAnimation == 1 && !game.pause) pacman.dieAnimation();
		if (game.pause !== true) {
 ...
 await game.init(0);game.started=true;game.pause=false;
 const imageReady=[pinky,inky,blinky,clyde].flatMap(g=>[g.image,g.dazzleImg,g.dazzleImg2,g.deadImg]).map(i=>i.decode().catch(()=>{}));
 await Promise.all(imageReady);
 const snap = obj => {const out={};for(const key of Object.keys(obj)){const v=obj[key];if(typeof v==='function'||v instanceof Element||key==='timer')continue;if(v && typeof v==='object')out[key]=Array.isArray(v)?JSON.parse(JSON.stringify(v)):snap(v);else out[key]=v;}return out};
 const merge=(target,source)=>{for(const key of Object.keys(source)){if(['__proto__','constructor','prototype','canvas','timer'].includes(key)||typeof target[key]==='function')continue;const v=source[key];if(v&&typeof v==='object'&&['up','down','left','right'].includes(v.name)&&'dirX' in v){target[key]=({up,down,left,right})[v.name];continue}if(v && typeof v==='object'&&!Array.isArray(v)){if(!target[key])target[key]={};merge(target[key],v)}else target[key]=v}};
 return {
  get score(){return game.score.score},get over(){return game.gameOver},get info(){return 'Nivel '+game.level+' / 10 · '+pacman.lives+' vidas'},
  async reset(){await game.init(0);game.started=true;game.pause=false;frame=0;renderContent()},
  action(key){const dirs={ArrowUp:up,ArrowDown:down,ArrowLeft:left,ArrowRight:right,w:up,s:down,a:left,d:right};if(dirs[key])pacman.directionWatcher.set(dirs[key])},
  update(dt){if(pendingLevel||game.gameOver)return;frame+=dt;while(frame>=game.refreshRate/1000){frame-=game.refreshRate/1000;animationLoop();if(pendingLevel)break;}},
  render(){canvas.width=canvas.width;renderContent()},
  save(){return {schema:1,game:'pacman',score:game.score.score,data:{game:snap(game),pacman:snap(pacman),ghosts:[pinky,inky,blinky,clyde].map(snap)}}},
  async load(p){if(p?.schema!==1||p.game!=='pacman'||!p.data?.game?.map)throw Error('Partida Pac-Man incompatible');await game.init(0);merge(game,p.data.game);merge(pacman,p.data.pacman);[pinky,inky,blinky,clyde].forEach((g,i)=>merge(g,p.data.ghosts[i]));game.started=true;game.pause=game.gameOver;game.buildWalls();renderContent()},
  pause(){},resize(){},key(key,down){if(down)this.action(key)}
 };
}
