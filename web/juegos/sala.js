...
function makeRoom(item){
 const card=document.createElement('section');card.id=item.id;card.className='game-card';card.setAttribute('data-appearance-node','game-card');card.innerHTML=`<div class="game-toolbar" data-appearance-node="toolbar"><button data-action="menu" aria-label="Volver al menú">‹ Menú</button><h2>${item.name}</h2><button data-action="sound" aria-pressed="true" aria-label="Sonido">♫</button><button data-action="full" aria-label="Pantalla completa">⛶</button></div><div class="stage" data-game="${item.id}"><canvas tabindex="0" aria-label="${item.name}, área de juego"></canvas><div class="hud"><span>0 pts</span><span></span></div><div class="controls" aria-label="Mando de ${item.name}"></div><div class="overlay"><strong>Preparando juego…</strong><span>Un momento.</span><button type="button" disabled>Jugar</button></div></div><div class="game-actions" data-appearance-node="controls"><button data-action="pause">▶ Jugar</button><button data-action="save">Guardar</button><button data-action="load">Cargar</button><button data-action="new">Nueva</button><button data-action="details" aria-expanded="false">Ayuda y récords</button></div><p class="save-status" role="status" data-appearance-node="status"></p><section class="game-details" data-appearance-node="surface" hidden><p class="help">${item.help}</p><h3>Récords entre empresas</h3><ol class="record-list"></ol><button class="refresh-rank">Actualizar</button></section>`;
...
for(const item of catalog){const b=document.createElement('button');b.className='game-choice';b.type='button';b.dataset.game=item.id;b.setAttribute('data-appearance-node','card');b.innerHTML=`<img src="/juegos/portadas/${item.id}.png" alt="Vista del juego ${item.name}" loading="lazy"><span class="choice-copy"><small>${item.tag}</small><strong>${item.name}</strong></span><span class="choice-play" aria-hidden="true">▶</span>`;b.onclick=()=>selectGame(item);menu.append(b)}
