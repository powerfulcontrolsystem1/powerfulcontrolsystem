(function () {
  ...
  function refreshAll() {
    return Promise.all([
      ...
      fetchJSON("config").then(function (row) { state.config = row || {}; renderConfig(); }),
      fetchJSON("categorias").then(function (rows) { state.categorias = rows || []; }),
      fetchJSON("activos").then(function (rows) { state.activos = rows || []; }),
      fetchJSON("tarifas").then(function (rows) { state.tarifas = rows || []; }),
      fetchJSON("contratos").then(function (rows) { state.contratos = rows || []; }),
      fetchJSON("mantenimientos").then(function (rows) { state.mantenimientos = rows || []; }),
      fetchJSON("ubicaciones").then(function (rows) { state.ubicaciones = rows || []; })
    ]).then(function () {
      syncSelects();
      renderTables();
      renderMap();
  ...
  function wireEvents() {
    ...
    byId("rentalSeedBtn").addEventListener("click", function () {
      sendJSON("seed_demo", {}).then(function () { return refreshAll(); }).then(function () { setMessage("Datos demo cargados.", false); }).catch(showError);
    });
