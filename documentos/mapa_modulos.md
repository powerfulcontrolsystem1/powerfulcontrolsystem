...
| Chat IA empresarial con agentes | Chat flotante/normal, `web/administrar_empresa/centro_ia_empresarial.html`, `web/administrar_empresa/estacion_ia_pedidos.html`, tarjetas IA de `web/administrar_empresa/estaciones.html` | `/api/empresa/chat_con_inteligencia_artificial/modelos`, `/consultar`, `/consultar_con_adjunto`, `/consultar_stream`, `/api/empresa/ia_pedidos_estacion/ejecutar`, `/api/empresa/ai/enterprise`, `backend/handlers/chat_con_inteligencia_artificial_controller.go`, `backend/handlers/ai_enterprise_orchestrator.go`, `backend/handlers/ia_pedidos_estacion.go` | `empresa_ai_uso_diario`, `empresa_ai_consultas`, `empresa_agentes_uso_diario`, `empresa_ai_conversaciones`, `empresa_ai_propuestas`, `empresa_ai_ejecuciones`; auditoria IA guarda identificadores operativos saneados, no secretos ni SQL libre | wrapper de ventas para establecer sesion/empresa y comprobacion adicional por herramienta contra permiso efectivo: hotel `reservas_hotel`, catalogo `inventario`; propuesta temporal, hash, vencimiento, idempotencia y confirmacion separada para escritura | Cargar catalogo de modelos/agentes, elegir agente general/ventas/inventario/compras/nomina/impuestos/agente_internet, probar consulta normal y streaming; en orquestador, validar `hotel.inspect_room_station` y `catalog.search_products`, preparar `hotel.configure_room_station` o `catalog.create_product`, confirmar una vez y comprobar auditoria; modificar `empresa_id` o usar permiso solo lectura debe ser rechazado |
