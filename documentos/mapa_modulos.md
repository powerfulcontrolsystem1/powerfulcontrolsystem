...
| Facturacion electronica Colombia | `web/administrar_empresa/facturacion_electronica.html`, `facturacion_electronica_pruebas_dian.html` como Centro de habilitacion, `facturacion_electronica_tutorial_dian.html` y `facturas_electronicas.html` | `/api/empresa/facturacion_electronica` para configuracion, documentos, reintentos, reconciliacion y artefactos; `/api/empresa/facturacion_electronica/dian` para credenciales, diagnostico, set real, TrackId y `GetStatusZip` | `facturacion_electronica_pais`, `facturacion_electronica_reintentos`, `empresa_dian_configuracion`, `empresa_dian_track_historial`, `empresa_facturacion_documentos` y `empresa_facturacion_artefactos`; sin preset numerico, objetivo exacto por empresa, CUFE/CUDE SHA-384 solo desde XML/acuse DIAN, consecutivo bajo `FOR UPDATE`, reintentos en `pcs-worker`, bloqueo asesor por empresa y documento, XML/acuse/PDF privados con SHA-256 y correo de produccion posterior a aceptacion | facturacion_electronica | Configurar empresa y objetivo exacto del portal, validar firma/credenciales sin exponer secretos, ejecutar set real solo con autorizacion, consultar `GetStatusZip` hasta acuse final, verificar artefactos y correo, probar nota credito sobre factura aceptada y confirmar aislamiento por `empresa_id` |
