...
| Cartera de proveedores (CxP) | `web/administrar_empresa/finanzas.html` | `/api/empresa/finanzas/cuentas_pagar`, `action=proveedores`, `backend/handlers/modulos_faltantes.go` | `empresa_cuentas_por_pagar`, `empresa_proveedores`, `empresa_cxp_pagos`, eventos/outbox | `finanzas`; wrapper `WithEmpresaFinanzasPermissions`; proveedor validado por `empresa_id` y activo | Crear proveedor, abrir CxP, seleccionar proveedor registrado, cargar soporte con IA, corregir el borrador, guardar, registrar abono idempotente, intentar proveedor de otra empresa/inhabilitado y confirmar rechazo, validar vista móvil e impresión |
