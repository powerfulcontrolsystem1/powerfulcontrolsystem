...
| Domotica | `Administrar empresa > Analisis y control > Domotica` en `web/administrar_empresa.html`, submenu `web/administrar_empresa/modulo_menu.html?module=control_electrico`, vistas `web/administrar_empresa/control_electrico.html?pagina=*` para resumen, conexion, controladores, estaciones/aparatos, automatizaciones, reportes y bitacora; tarjeta `Domotica` en `web/administrar_empresa/carrito_de_compras.html`; storage y transferencia en `web/super/domotica_storage.html` y `web/super/domotica_raspberry_trafico.html` | `/api/empresa/control_electrico`, `/api/public/domotica/tunnel`, `/super/api/domotica_storage`, `/super/api/domotica_raspberry_trafico`, `backend/handlers/control_electrico*.go` | `empresa_control_electrico_config`, `empresa_control_electrico_raspberry_pis`, `empresa_control_electrico_reles`, `empresa_control_electrico_eventos`, `empresa_control_electrico_lecturas`, `empresa_control_electrico_reglas`, `empresa_control_electrico_comandos`, `empresa_control_electrico_trafico_diario`, configs `domotica.storage.*` | clave tecnica `control_electrico`; configuracion y provision requieren permiso efectivo `A`; el endpoint publico usa identidad de dispositivo y no acepta `empresa_id`; paginas super solo super admin | Desde la Raspberry abrir PCS, registrar controlador, descargar y ejecutar instalador, comprobar tunel conectado; crear varios aparatos por estacion con descripcion/foto/watts/horario; probar salida GPIO, regla de entrada, historial y transferencia por dispositivo |
