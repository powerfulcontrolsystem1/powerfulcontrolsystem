...
Actualizacion 2026-08-25: el Chat IA empresarial usa un drawer simple y
multimodal compartido por `web/js/ai_chat_drawer.js`. El botón flotante retira
`hidden` antes de abrir. `/historial` devuelve por defecto solo consultas de la
cuenta autenticada y acepta `scope=empresa` únicamente para
`super_administrador`, `administrador_total` o `admin_empresa`, siempre con
`empresa_id` validado. Super Administrador selecciona un modelo global único
para texto y adjuntos; el valor inicial es `openai:gpt-5.6-terra`, esfuerzo
`medium`.

Actualizacion 2026-08-25: se retira el interruptor `Modo agente` y cualquier
selector visible de agentes/modelos en el drawer, Centro IA y Pedidos IA. El
backend fuerza el único `agente_pcs`; las preguntas normales usan la cuota del
chat y una cuota de herramienta solo se reserva si ocurre una llamada real.
...
- UI: el submenu de Domótica enlaza `control_electrico.html?pagina=raspberry` para elegir uso/cantidad/delay, `configuracion_sensores_raspberry.html` para asociar OUT/IN a habitaciones y `tutorial_sensores_puertas_raspberry.html` para cableado BCM/pin físico, matriz 4×16 y seguridad de 3,3 V.
- API empresarial: `/api/empresa/control_electrico?action=raspberry_pi|provisionar_tunel` bajo permiso administrativo; `/api/empresa/sensor_puertas` conserva el mapeo por empresa.
- Tunel: `/api/public/domotica/tunnel?action=poll|door_scan` autentica dispositivo, nunca acepta `empresa_id` y valida 1–16 salidas por cuatro entradas.
- Datos: `empresa_control_electrico_raspberry_pis`, `empresa_sensor_puertas_devices`, `empresa_sensor_puertas_messages` y `empresa_control_electrico_eventos`.
- QA: seleccionar modo puertas, confirmar 4N canales, asociar un canal, render de estación, auditoría antes/después, rechazo cross-tenant y prueba física con 3,3 V.

## Actualizacion 2026-08-23 - portal publico y catalogo vigente

| Superficie | Frontend | API / backend | Fuente de catalogo | Validacion |
|---|---|---|---|---|
| Portada publica | `web/index.html` | `/api/public/pagina_principal`, `/api/public/informacion_de_modulos`; `backend/handlers/pagina_principal_handlers.go` | 52 modulos de permiso en el corte de esa fecha; cuatro plantillas clasicas y nueve nuevas | Las tarjetas persistidas retiradas se descartan en backend y frontend; las 13 plantillas se agregan sin duplicar claves de modulo |
| Landing descriptiva | `web/descripcion_de_los_sistemas.html` | `/api/public/pagina_principal`, `/api/public/plantillas_nuevas/catalogo` | Mismo catalogo de la portada | Encabezado vigente, cuatro fichas clasicas y nueve nuevas; no publica destinos retirados |
| Compatibilidad de landing | `web/descripcion_de_los_sistemas.ht` | Contrato igual a la landing canonica | Mismo catalogo de la portada | Conserva la ruta heredada como HTML y aplica el mismo filtro defensivo |
| Editor de portada | `web/super/pagina_principal.html` | `/super/api/pagina_principal` | Hasta 12 tarjetas personalizadas | Al cargar y guardar recibe la configuracion normalizada e informa el corte vigente |

## Actualizacion 2026-08-31 - Vida personal

Vida eleva el catalogo actual a 53 modulos de permiso y conserva las 13
plantillas empresariales.

| Superficie | Frontend | API / backend | Persistencia | Aislamiento |
|---|---|---|---|---|
| Resumen y gastos | `web/administrar_empresa/vida.html`, `web/js/vida.js` | `/api/empresa/vida` acciones `dashboard`, `gastos`, `gasto` y `recibo` | `empresa_vida_gastos` y archivos bajo la categoria privada `vida` | Todas las lecturas y mutaciones usan empresa y cuenta autenticada |
| Suscripciones | Misma pagina, pestana `Suscripciones` | Acciones `suscripciones`, `suscripcion` y `renovar` | `empresa_vida_suscripciones` | No existe vista global ni acceso administrativo a registros ajenos |
| Recordatorios | Tarjetas de Vida, contador del menu y Notification API opcional | La API deriva alertas de la fecha y anticipacion configuradas | Sin proveedor externo ni envio autonomo | La deduplicacion del navegador incluye empresa, suscripcion y fecha |

## Actualizacion 2026-08-24 - Administrar disco VPS

| Superficie | Frontend | API / backend | Persistencia | Validacion |
|---|---|---|---|---|
| Administrar disco VPS | `web/super/administrar_disco_vps.html` desde `web/super_administrador.html` | `/super/api/administrar_disco_vps`, `backend/handlers/super_disk_manager.go`, sidecar interno `pcs-disk-manager` | No crea tablas ni guarda rutas o IDs Docker | Solo `super_administrador`; confirmación exacta `LIBERAR ESPACIO`; la limpieza acepta únicamente contenedores detenidos, imágenes sin uso, caché BuildKit vencida y volúmenes anónimos sin referencias |
