...
`nomina_sueldos.html?seccion=...`; el boton historico prepara el lote, pero el
endpoint `/api/empresa/facturacion_electronica?action=nomina_electronica`
responde 422 sin generar ni enviar XML hasta que el modulo tenga el adaptador
DIAN propio de nomina.
...
| Facturacion electronica Colombia | `web/administrar_empresa/facturacion_electronica.html`, `facturacion_electronica_pruebas_dian.html` como Centro de habilitacion, `facturacion_electronica_tutorial_dian.html`, `facturas_electronicas.html`, `administrar_clientes.html` y los flujos especializados de nómina/documento soporte en `contabilidad_colombia_avanzada.html` + `web/js/documento_soporte_dian.js` | `/api/empresa/facturacion_electronica` para configuracion, documentos, reintentos, reconciliacion, artefactos, `action=anular_factura_nota_credito`, `action=configuracion_documentos_dian` y emisión dedicada `action=emitir_documento_soporte`; `/api/empresa/facturacion_electronica/dian` para credenciales, diagnostico, set real, TrackId y `GetStatusZip`; prevalidaciones sin emisión en `/api/empresa/contabilidad_colombia_avanzada?action=nomina_electronica_preflight|documento_soporte_preflight` | `facturacion_electronica_pais`, `facturacion_electronica_reintentos`, `empresa_dian_configuracion`, `empresa_dian_documentos_configuracion`, `empresa_dian_track_historial`, `empresa_facturacion_documentos`, `empresa_facturacion_fuentes_fiscales`, `empresa_facturacion_artefactos` y borradores `empresa_contabilidad_nomina_electronica`/`empresa_contabilidad_documentos_soporte`; factura, nota crédito total y documento soporte conservan fuentes fiscales separadas e inmutables. Documento soporte agrega líneas/vendedor/pago, montos exactos, instantánea de numeración y espejo CUDS/acuse por `empresa_id`; todos los XML/acuse/PDF son privados con SHA-256 | facturacion_electronica para configuración/emisión/artefactos; contabilidad_colombia_avanzada para borrador/preflight | Configurar emisor/adquirente y vendedor con datos fiscales completos; validar firma sin exponer secretos; revisar documento soporte sin efectos; confirmar por frase solo un borrador real listo; comprobar que una configuración/rango incompleto no consume consecutivo. La factura y nota crédito total conservan sus flujos. Nota crédito libre/parcial, nota débito, nota de ajuste de soporte, nómina, equivalentes, contingencia y RADIAN permanecen bloqueados hasta sus adaptadores/fuentes propios |
