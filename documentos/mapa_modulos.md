...
El menú flotante agrupa Calculadora y Juegos en Accesorios. Son las únicas
entradas del shell empresarial que abren una ventana independiente. Los seis juegos
...
`Administrar usuarios > Roles personalizados > Permisos` edita acciones y páginas
del ID propio mediante `roles_de_usuario?action=permisos&rol_id=...`. El motor
combina base, matriz propia y límites empresariales para la identidad de la sesión.
La búsqueda no descarta filas y los selectores conservan IDs históricos distintos.
La matriz global es informativa; el contexto de la petición decide la autorización.
Esta regla reemplaza las referencias a herencia exclusivamente del rol base de
la fila histórica de usuarios. APIs, fuentes y nuevas pruebas están enlazadas en el
...
servidor, exige perfil fiscal y pagos reales, se sella con una configuracion de
familia independiente y usa reserva atomica, CUNE SHA-384, XAdES, XSD oficial y
`SendNominaSync`. Emision/reenvio y lectura de datos fiscales de nomina cruzan
los permisos efectivos de Facturacion y Nomina; listados generales omiten esa
familia para usuarios sin permiso de Nomina. Habilitacion automatica por
`SendTestSetAsync`, `NominaIndividualDeAjuste` y entrega/PDF dedicado siguen
bloqueados. El candidato aun no ha sido desplegado ni emitido contra DIAN.

Actualizacion 2026-08-26 - Documento soporte DIAN 1.1: el submodulo de
`contabilidad_colombia_avanzada` crea borradores estructurados de compras a no
obligados y ofrece un preflight de solo lectura. La emision dedicada exige
configuracion propia por empresa, reserva atomica e idempotente, fuente fiscal
sellada, confirmacion escrita, CUDS SHA-384, XAdES y `SendBillSync`. El prefijo
de numeracion es opcional y el detalle admite el catalogo exacto de 1.093
unidades UN/ECE Revision 4 publicado dentro de la caja DIAN 1.1. Este cambio no
abre emision libre en el Centro DIAN ni habilita nota de ajuste de soporte,
nomina, equivalentes, contingencia o RADIAN. Merge, despliegue y primer acuse
real se documentan como compuertas separadas.
...
nota de ajuste de soporte, nota debito, equivalentes, contingencia y RADIAN
siguen cerrados; documento soporte solo opera desde su borrador contable
estructurado y adaptador dedicado descrito en la actualización 2026-08-26.
...
CUFE/fecha y solo anula la factura tras acuse aceptado. El catalogo la muestra
como capacidad parcial; emision libre/parcial y nota debito siguen bloqueadas.

Actualizacion 2026-08-25 - Factura real y respuesta sincronica DIAN: el pago
expone `factura_electronica` en el nivel superior y conserva compatibilidad con
el contrato anidado; la UI presenta numero legal y avisos fiscales. Cuando
...
pendientes con bloqueo por `empresa_id`. `empresa_facturacion_artefactos`
registra XML firmado, acuse y PDF en almacenamiento privado con SHA-256; la
bandeja usa `action=artefactos|descargar_artefacto` sin exponer `storage_ref`.
Esta actualizacion reemplaza las referencias historicas a 30/10/10 y a archivos
fiscales bajo `web/uploads` que aun aparezcan en la fila extensa del modulo.
...
retiro con los documentos fiscales DIAN para juegos de suerte y azar, que son
categorias legales independientes.
...
`nomina_sueldos.html?seccion=...`. Esta entrada es historica: el preflight
continua sin efectos, pero la actualizacion 2026-08-26 agrega el adaptador
dedicado `NominaIndividual` y la emision controlada exclusivamente desde
`action=emitir_nomina_electronica`.
...
preparado para una conciliacion automatica futura por webhook/API bancaria
homologada, pero esa opcion permanece deshabilitada; mientras tanto se registra
la referencia como pendiente y el cajero verifica el abono en el banco.
...
Para varias cajas simultaneas se usa referencia unica por empresa/caja/carrito o
QR dinamico generado exclusivamente por la API oficial del participante; la
confirmacion automatica no esta disponible sin webhook/API bancaria homologada.
...
| Estaciones | `web/administrar_empresa/estaciones.html` | `/api/empresa/carritos_compra`, `/api/empresa/carritos_compra/items`, `/api/empresa/estacion_prefs`, `/api/empresa/estacion_aseo` | estaciones en `empresa_estacion_prefs`; cada entrada admite `mostrar_fecha_hora_inicio`, `mostrar_fecha_hora_fin_tarifa` y `mostrar_cliente_nombre`; `acceso_estaciones_cajeros.usuarios[email].caja_codigo`, `cajas_config[].estaciones/modo_estaciones`, carritos, `empresa_estacion_aseo_eventos`, lectura de `carrito_ui_global.modo_pantalla_tactil` | ventas/estaciones; acceso efectivo es interseccion caja/usuario; `portero` o caja `solo_activar` restringidos en carritos; `servicio_limpieza` con `ventas:R` y finalizacion de aseo | Activar estacion, comprobar que inicio, fin tarifario y cliente respetan sus tres checks individuales, doble clic, estados entre usuarios, modo POS tactil con tarjetas grandes, dos cajas con rangos distintos, cajero sin acceso recibe 403 por URL/API, caja `solo_activar` no abre ni cobra carrito, portero respeta además su filtro, Servicio de limpieza solo limpia estaciones sucias |
| Carrito y venta directa | `web/administrar_empresa/carrito_de_compras.html`; estilos finales en `web/estilos.css` bajo `body.carrito-flat-page` y `body.pos-touch-mode`; iframe de `web/administrar_empresa.html` permite fullscreen | `/api/empresa/carritos_compra`, `/api/empresa/carritos_compra/items`, `action=transferir_estacion`, auxiliares `/api/empresa/productos`, `/api/empresa/servicios`, `/api/empresa/recetas_productos`, `/api/empresa/clientes`, `/api/empresa/creditos?action=disponibilidad_cliente|limites_cliente`, `/api/empresa/codigos_de_descuento`, `/api/empresa/propinas`, `/api/empresa/comisiones` | `carritos_compras`, `carrito_compra_items`, abonos, `inventario_existencias`, `inventario_movimientos`, `clientes`, `empresa_creditos`, `empresa_creditos_clientes_limites`, `empresa_propinas_movimientos`, `empresa_comisiones_servicio_*`, `empresa_estacion_prefs`; `estaciones_config.estaciones[]` separa mesero y comisionista y `carrito.comisionista_ultimo` recuerda la seleccion por estacion; la comision puede usar solo servicios o servicios mas productos aunque todos se facturan juntos; la propina queda fuera del total fiscal; tarjeta de bascula usa Web Serial solo cuando esta habilitada y atajos POS se leen desde `carrito_ui_global.atajos_pos` | ventas/carritos; cajero con APIs auxiliares sin paginas administrativas; transferencia bloqueada si el check empresarial esta apagado; mesero y comisionista se validan contra `users` del mismo `empresa_id` | Agregar, editar y devolver items; validar inventario, cliente, credito, descuentos, pagos simples/mixtos, QR, transferencia entre estaciones, tarifa, bascula, atajos, impresion e idempotencia; verificar que `Selecciona una accion` y `Domotica` queden a la derecha de `Buscar producto` en escritorio y se reorganicen sin desborde en movil; asignar mesero y comprobar reporte de propinas por persona/fecha; mostrar u ocultar comisionista por estacion, seleccionar y recordar el ultimo; cobrar con `incluir_productos` apagado/encendido y verificar bases; alterar usuario o `empresa_id` y comprobar rechazo |
| Domotica | `Administrar empresa > Analisis y control > Domotica` en `web/administrar_empresa.html`, submenu `web/administrar_empresa/modulo_menu.html?module=control_electrico`, vistas `web/administrar_empresa/control_electrico.html?pagina=*`; el reporte incluye seguimiento filtrable por estación, equipo, Raspberry, evento, resultado y fecha; boton unico `⚡ Domotica` por tarjeta en `web/administrar_empresa/estaciones.html`, configurado en `web/administrar_empresa/configuracion_de_estaciones.html`, que abre `web/administrar_empresa/carrito_control_electrico.html`; storage y transferencia en `web/super/domotica_storage.html` y `web/super/domotica_raspberry_trafico.html` | `/api/empresa/control_electrico`, `/api/public/domotica/tunnel`, `/super/api/domotica_storage`, `/super/api/domotica_raspberry_trafico`, `backend/handlers/control_electrico*.go` | `empresa_control_electrico_config`, `empresa_control_electrico_raspberry_pis`, `empresa_control_electrico_reles`, `empresa_control_electrico_eventos`, `empresa_control_electrico_lecturas`, `empresa_control_electrico_reglas`, `empresa_control_electrico_comandos`, `empresa_control_electrico_trafico_diario`, `estaciones_config.station_card_ui.mostrar_boton_domotica`, configs `domotica.storage.*` | clave tecnica `control_electrico`; configuracion y provision requieren permiso efectivo `A`; el endpoint empresarial filtra eventos y valida IDs secundarios con `empresa_id`; el endpoint publico usa identidad de dispositivo y no acepta `empresa_id`; paginas super solo super admin | Desde la Raspberry abrir PCS, registrar un controlador con instalador exclusivo, comprobar reconexion/tunel; en Estaciones activar el check y abrir el boton por habitacion; verificar equipos, sensores, estados y multiples Raspberry; filtrar el seguimiento por equipo/estación/Raspberry/resultado/fecha y confirmar que una empresa no ve eventos de otra; probar salida GPIO solo con cableado confirmado, regla de entrada, historial y transferencia por dispositivo |
...
| Chat IA empresarial | Chat principal flotante/normal y `web/administrar_empresa/centro_ia_empresarial.html` | `/api/empresa/chat_con_inteligencia_artificial/modelos`, `/consultar`, `/consultar_con_adjunto`, `/consultar_stream`, `/api/empresa/ai/enterprise`, `backend/handlers/chat_con_inteligencia_artificial_controller.go`, `backend/handlers/ai_enterprise_orchestrator.go` | `empresa_ai_uso_diario`, `empresa_ai_consultas` por `empresa_id` y usuario, `empresa_ai_usuario_preferencias`, `empresa_ai_memoria`, `empresa_ai_conversaciones`, `empresa_ai_propuestas`, `empresa_ai_ejecuciones`; las propuestas de tarifa escriben, tras confirmacion, en `empresa_tarifas_por_minutos`, `empresa_tarifas_por_dia` y `estaciones_config` sin tablas paralelas | `WithEmpresaAIEnterprisePermissions` valida usuario y tenant; historial/preferencias se filtran por empresa y usuario. La UI no selecciona agentes. `tariffs.configure_minutes_station` y `hotel.configure_room_station` exigen rol administrativo, `ventas:U`, flags de escritura, propuesta temporal, hash, vencimiento, idempotencia y confirmacion separada | Abrir el chat principal con administrador y preparar tarifas semanal/minutos o amanecida; revisar resumen y confirmar una vez. Repetir la confirmacion debe ser idempotente. Con cajero, solo lectura o `empresa_id` ajeno, la herramienta no debe exponerse ni ejecutarse |
| Ayuda con caja IA | `web/js/help_ai_bridge.js`, `web/js/ai_chat_drawer.js`, ayudas `web/ayuda/*.html`, tutoriales empresariales | Usa `PCSAIChatHelp`, compatibilidad `PCSAIChatRobot.showMessage` redirigida al recuadro normal, mensaje frontend `pcs-help-ai-open`, chat `/api/empresa/chat_con_inteligencia_artificial/*` y radio `/api/empresa/ia_radio/activar` | Preferencias `chat_flotante.*` en `empresa_estacion_prefs`; `chat_flotante.chat_enabled` queda activo para empresas y se reaplica en arranque; `ia_radio` solo cambia emisora por empresa; los enlaces de ayuda conservan su URL HTML como respaldo y el puente conserva `empresa_id` en URLs internas; el boton `Stop` cancela audio y respuesta activa con `AbortController`/secuencia local sin modificar datos | El asistente se muestra como recuadro normal con icono circular flotante visible en empresas; robot/secretaria quedan retirados; la voz se conserva; productos, nomina, estaciones y tarifas dependen de herramientas registradas, wrappers y permisos | Abrir Nomina, presionar `Ayuda`, confirmar que dentro del panel empresarial aparece el recuadro IA con ayuda contextual, que el icono circular no desaparece por cache viejo, que `Stop` corta audio/respuesta en curso y que una accion `PCS_ACTION` requiere confirmacion antes de ejecutar |
...
| Facturacion electronica Colombia | `web/administrar_empresa/facturacion_electronica.html`, `facturacion_electronica_pruebas_dian.html` como Centro de habilitacion, `facturacion_electronica_tutorial_dian.html`, `facturas_electronicas.html`, `administrar_clientes.html`; flujos especializados en `contabilidad_colombia_avanzada.html`, `web/js/documento_soporte_dian.js` y `nomina_sueldos.html` | `/api/empresa/facturacion_electronica` para configuracion, documentos, reintentos, reconciliacion, artefactos, `action=anular_factura_nota_credito`, `action=configuracion_documentos_dian`, `action=emitir_documento_soporte` y `action=emitir_nomina_electronica`; `/api/empresa/facturacion_electronica/dian` para credenciales, diagnostico, set real de factura, TrackId y `GetStatusZip`; preflight sin emision en `/api/empresa/nomina?action=nomina_electronica_preflight` y `/api/empresa/contabilidad_colombia_avanzada?action=documento_soporte_preflight` | `facturacion_electronica_pais`, `facturacion_electronica_reintentos`, `empresa_dian_configuracion`, `empresa_dian_documentos_configuracion`, `empresa_dian_track_historial`, `empresa_facturacion_documentos`, `empresa_facturacion_artefactos`, `empresa_contabilidad_documentos_soporte`, `empresa_contabilidad_nomina_electronica` y `empresa_nomina_dian_perfiles`; factura, nota credito total, documento soporte y nomina ordinaria conservan fuentes fiscales separadas e inmutables. XML/acuse son privados con SHA-256; factura/documento soporte conservan PDF propio, mientras nomina bloquea el PDF/correo generico | facturacion_electronica para configuracion/emision/artefactos; nomina_sueldos adicional para datos, preflight, lectura y emision de nomina | Configurar emisor, software/proveedor, firma y cada familia; validar factura, anulacion total y documento soporte en sus fuentes propias; para nomina usar mes cerrado, perfil fiscal, liquidaciones/pagos reales, preflight, doble permiso y frase fuerte. Habilitacion automatica de nomina, nota de ajuste de soporte/nomina, nota debito, equivalentes, contingencia y RADIAN permanecen bloqueados |
...
| Cartera de proveedores (CxP) | `web/administrar_empresa/finanzas.html` | `/api/empresa/finanzas/cuentas_pagar`, `action=proveedores`, `backend/handlers/modulos_faltantes.go` | `empresa_cuentas_por_pagar`, `proveedores` (catálogo operativo), `empresa_cxp_pagos`, eventos/outbox | `finanzas`; wrapper `WithEmpresaFinanzasPermissions`; proveedor validado por `empresa_id` y activo | Crear proveedor, abrir CxP, seleccionar proveedor registrado, cargar soporte con IA, corregir el borrador, guardar, registrar abono idempotente, intentar proveedor de otra empresa/inhabilitado y confirmar rechazo, validar vista móvil e impresión |
| Recuperación operacional outbox CxP | `web/super/administrar_base_de_datos.html` | `/super/api/outbox/recovery`, `backend/handlers/super_outbox_recovery.go` | `pcs_outbox_events`, `pcs_outbox_recovery_audit` | Solo `super_administrador`; `empresa_id`, topic CxP e IDs explícitos obligatorios; CSRF, frase de confirmación y auditoría | Vista previa PCS, rechazo de otro tenant/topic/estado, reactivación controlada, worker idempotente, un evento/asiento por pago y auditoría |
...
| Reportes ejecutivos y auditoria de modulos | `web/administrar_empresa/reportes_ejecutivos.html`, filtro ampliado en `web/administrar_empresa/auditoria.html` | `/api/empresa/reportes`, dataset `operativo_modulos_resumen`; `/api/empresa/auditoria/eventos` para eventos y cadena de custodia | datos operativos por modulo; inventario de tablas empresariales, hubs sin tabla y actividad por `empresa_id`; eventos especificos para Bre-B QR, buzon/tareas/chat, impresoras, menu visible y atajos POS | reportes; rol `empresario` con `reportes:R`; auditoria `R` para consultar/exportar y `D` para `retener|purgar` | Select de reporte, abrir `Auditoria de modulos`, filtrar area `Operacion y auditoria`, exportar vista o forense, validar DIAN/Bolsa/Suite contador/NIIF/Centro IA/OnlyOffice/Bre-B/Buzon/Impresoras/Menu visible/Atajos POS en filtros y que `empresario` no vea reportes de turnos/caja |
...
| Ubicacion GPS | Grupo principal propio en `web/administrar_empresa.html`; seguimiento y configuracion en `web/administrar_empresa/ubicacion_gps.html` | `/api/empresa/ubicacion_gps/dispositivos`, `/api/empresa/ubicacion_gps/recorridos`, `backend/handlers/ubicacion_gps.go` | `empresa_gps_dispositivos`, `empresa_gps_recorridos` | `ubicacion_gps:C`, paginas `linkUbicacionGPS` y `linkConfiguracionGPS`; wrapper `WithEmpresaUbicacionGPSPermissions` | Abrir ambos accesos del grupo GPS, comprobar seleccion de seccion, configurar un dispositivo autorizado, consultar mapa/recorrido y rechazar IDs de otra empresa |
...
| Nomina | `web/administrar_empresa/nomina_sueldos.html`, tutorial operativo `web/administrar_empresa/nomina_tutorial.html`, ayuda guiada `web/ayuda/tutorial_nomina.html` | `/api/empresa/nomina` acciones `config`, `parametros_legales*`, `empleados`, `liquidaciones`, `pagos`, `dashboard*`, `perfil_dian`, `documentos_electronicos_colombia`, `nomina_electronica_preflight`, `preparar_nomina_electronica`; emision dedicada por `/api/empresa/facturacion_electronica?action=emitir_nomina_electronica` | `empresa_nomina_configuracion` con `periodo_nomina_dian`, parametros legales, `empresa_nomina_empleados`, liquidaciones/pagos, `empresa_nomina_colombia_*`, `empresa_nomina_dian_perfiles`, `empresa_contabilidad_nomina_electronica`; un documento mensual por empleado sella fuente/configuracion y se refleja en documentos, reintentos y artefactos fiscales | nomina/finanzas por `empresa_id`; emision y reenvio requieren ademas aprobacion de facturacion, y consultas/artefactos fiscales requieren lectura en ambos dominios | Configurar datos legales y perfil DIAN, validar sedes, liquidar/conciliar, registrar pagos reales, elegir mes cerrado, ejecutar preflight sin efectos y comprobar bloqueos. No usar demo para una prueba fiscal; confirmar emision solo con fuente/configuracion genuinas. Validar que ajuste, habilitacion automatica y correo/PDF generico sigan bloqueados; abrir Tutorial/Ayuda conservando `empresa_id` |
...
| Auditoría de inicios de sesión | `web/super/auditoria_login.html`, `web/js/super_auditoria_global.js` | `GET /super/api/auditoria?scope=super_panel&modulo=autenticacion`; productores en `AdminLoginHandler`, `EmpresaUsuarioLoginHandler` y callback Google | `super_auditoria_eventos`, sin contraseñas ni tokens | super_administrador | Probar acceso exitoso y rechazado de administrador/usuario, filtrar por empresa/identidad/resultado, abrir detalle y exportar CSV/JSON |
...
| Apariencias / menu flotante | `web/menu.js`, paginas con selector de tema, calculadora, campana global de notificaciones y tickets | `/api/user/configuracion`; consume `/api/empresa/buzon?action=resumen` y marca leido con `action=leer` por `empresa_id` | `usuario_configuracion.apariencia`; notificaciones de buzon por `empresa_id` | Sesion autenticada cuando guarda en servidor; lectura visual de notificaciones empresariales; Juegos personales disponibles desde Accesorios con clasificación recreativa global | Cambiar tema en escritorio y movil; validar iframes con `dark-corporate`, `dark-absolute`, `dark-obsidian` y `light`; verificar Calculadora y Juegos dentro de Accesorios; Calculadora y Juegos abren ventanas independientes sin reemplazar el panel; Juegos y sus creditos exponen nodos claros/oscuros con contraste, foco y superficies adaptables, que la campana sea el primer item del panel y que el clic navegue al enlace relacionado |
