...
| Facturacion electronica Colombia | `web/administrar_empresa/facturacion_electronica.html`, `facturacion_electronica_pruebas_dian.html` como Centro de habilitacion, `facturacion_electronica_tutorial_dian.html`, `facturas_electronicas.html`, `administrar_clientes.html` y las revisiones de nómina/soporte en `contabilidad_colombia_avanzada.html` | `/api/empresa/facturacion_electronica` para configuracion, documentos, reintentos, reconciliacion, artefactos y `action=configuracion_documentos_dian`; `/api/empresa/facturacion_electronica/dian` para credenciales, diagnostico, set real, TrackId y `GetStatusZip`; prevalidaciones sin emisión en `/api/empresa/contabilidad_colombia_avanzada?action=nomina_electronica_preflight|documento_soporte_preflight` | `facturacion_electronica_pais`, `facturacion_electronica_reintentos`, `empresa_dian_configuracion`, `empresa_dian_documentos_configuracion`, `empresa_dian_track_historial`, `empresa_facturacion_documentos`, `empresa_facturacion_artefactos` y borradores `empresa_contabilidad_nomina_electronica`/`empresa_contabilidad_documentos_soporte`; la factura comercial conserva `fuente_fiscal_json` inmutable con líneas/partes reales, exige códigos DANE, usa consecutivo bajo `FOR UPDATE`, bloqueo asesor por empresa/documento, XML/acuse/PDF privados con SHA-256 y correo posterior a aceptación | facturacion_electronica | Configurar emisor/cliente con DANE y objetivo exacto del portal, validar secretos/firma sin exponerlos, ejecutar set real solo con autorización, consultar `GetStatusZip` hasta acuse final y confirmar aislamiento por `empresa_id`; nota crédito/débito, nómina, soporte, equivalentes, contingencia y RADIAN permanecen bloqueados hasta sus adaptadores propios |
