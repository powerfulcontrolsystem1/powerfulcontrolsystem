...
| `/api/empresa/soportes_compras_ia` | `WithEmpresaSoportesComprasIAPermissions` | SA, AE, SS, CO, CT | - | captura foto/PDF/XML de compras y gastos; `dashboard|soportes|eventos|retencion_preview` usa `R`, `extraer_ia|editar_revision|restaurar` usa `U`, `aprobar|rechazar|contabilizar` exige `A` y `eliminar|purgar` exige `D`; purgar requiere retencion, motivo y codigo exacto |
