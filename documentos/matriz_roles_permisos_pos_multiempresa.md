...
| `/api/empresa/soportes_compras_ia` | `WithEmpresaSoportesComprasIAPermissions` | SA, AE, SS, CO, CT | - | captura foto/PDF/XML de compras y gastos; `extraer_ia|editar_revision|restaurar` usa `U`, `aprobar|rechazar|contabilizar` exige `A` y `eliminar` exige `D` |
