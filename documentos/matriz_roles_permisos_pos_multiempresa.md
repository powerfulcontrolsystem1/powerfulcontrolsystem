...
| `/api/empresa/facturacion_electronica` | `WithEmpresaFacturacionPermissions` | SA, AE, CJ | - | `action=emitir|emitir_factura|emitir_documento` exige `A`; `nota_credito` y `emitir_nota_credito` permanecen autorizadas por rol pero bloqueadas HTTP 422 hasta implementar su fuente fiscal y adaptador DIAN propios |
