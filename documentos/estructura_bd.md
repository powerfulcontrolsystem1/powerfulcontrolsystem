...
- `empresa_nextcloud_accounts` mantiene una fila por `empresa_id`, usuario
  tecnico unico, cuota, estado y fecha de aprovisionamiento.
- La tabla referencia `empresas(id)` con borrado en cascada y no almacena la
  contrasena del usuario. La credencial temporal existe solo en memoria durante
  la respuesta que crea o restablece la cuenta.
- El secreto administrativo OCS se guarda cifrado en la configuracion super.
