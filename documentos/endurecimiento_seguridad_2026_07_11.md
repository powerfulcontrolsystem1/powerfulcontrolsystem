...
| Binarios compilados vulnerables | Crítica | Trivy detectó ejecutables Go antiguos, Grype encontró `ngrok.exe` con Go 1.16.2 y el paquete RustDesk local con Go 1.23.4 vulnerable | retiro de binarios generados/locales y de instaladores opacos; RustDesk se obtiene desde su canal oficial y `/app/descargas` queda como volumen runtime | Trivy de imagen y Grype sobre SBOM en GitHub Actions | pendiente del run posterior al commit | verificar checksum de cada instalador externo antes de publicarlo | en validación |
