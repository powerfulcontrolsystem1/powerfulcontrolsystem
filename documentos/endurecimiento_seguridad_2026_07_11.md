...
| Binarios e instalador RustDesk heredados | Crítica | Trivy detectó ejecutables Go antiguos; Grype encontró `ngrok.exe`, instaladores RustDesk y `tools/install_rustdesk` con runtimes vulnerables; el último descargaba `master/setup.sh` y lo ejecutaba como root sin checksum | retiro de binarios generados/locales, instaladores opacos y del instalador no fijado; RustDesk operativo conserva contenedores/versiones auditadas y la UI usa el canal oficial | Trivy de imagen y Grype sobre SBOM, run 29217611192 | pasa | verificar checksum de cada artefacto externo antes de publicarlo | corregido |
| Base Nginx obsoleta | Alta | imagen 1.27 incluía bibliotecas Alpine vulnerables y el primer rebuild conservó `c-ares 1.34.6-r0` | Nginx unprivileged 1.30.3 sobre Alpine 3.23 y `c-ares 1.34.8-r0`, versiones fijas | construcción y Trivy de imagen, run 29217611192 | pasa | fijar digest tras validar la arquitectura de producción | corregido |
| Cadena de suministro incompleta | Alta | CI sin secretos/IaC/SBOM/imágenes | jobs separados con herramientas versionadas, gates explícitos, SARIF y artefactos | GitHub Actions 29217611192 | secretos, filesystem, IaC, imágenes, SBOM y licencias pasan | bases de vulnerabilidades cambian | corregido |
