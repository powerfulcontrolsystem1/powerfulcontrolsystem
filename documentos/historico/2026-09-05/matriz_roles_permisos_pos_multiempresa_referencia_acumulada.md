...
- El panel `web/super/permisos_rol.html` configura la **matriz por rol** (módulo x R/C/U/D/A) y **anulaciones por función** del menú `administrar_empresa` (claves `link*`). El backend expone en `GET /super/api/roles_de_usuario/permisos` etiquetas legibles y agrupación para auditoría y UI.
...
- No se agrega un tercer sistema -Å“universal- paralelo: la combinación licencia + rol + reglas de página del catálogo `permissionPagesCatalogOrdered` en `empresa_permisos.go` es el modelo soportado.
...
- Actualizacion 2026-04-20.2 (soporte remoto: tope diario RustDesk):
	- La edición del nuevo campo `max_minutos_dia_rustdesk` se concentra en `/super/api/soporte_remotoaction=config` y su vista super asociada.
...
|---|---|---|---|---|
| `/api/empresa/vida` | `WithEmpresaVidaPermissions` | todos los roles empresariales autenticados, solo sobre su cuenta | todos los roles empresariales autenticados, solo sobre su cuenta | `reporte` y preferencias de notificación siempre derivan `usuario_id` de la sesión; WhatsApp no retorna el número completo |
...
| `/api/empresa/control_electrico` | `WithEmpresaControlElectricoPermissions` | SA, AE, SS | SA, AE | configuracion Domotica por empresa: controladores, aparatos, fotos, lecturas, reglas de sensores, alarmas, reportes, sincronizacion y generacion de instalador; cambios, comandos y provision requieren permiso efectivo `A` |
| `/api/public/domotica/tunnel` | identidad `device_uid` + Bearer de dispositivo | Raspberry enrolada | Raspberry enrolada | no usa sesion humana ni acepta `empresa_id`; la identidad hash resuelve empresa y controlador, con body limitado y respuesta `no-store` |
| `/super/api/domotica_raspberry_trafico` | `WithSuperAuditoria` + sesion super | SA | no | transferencia RX/TX acumulada y diaria por empresa/Raspberry; no expone tokens |
...
- `/super/pagos_auditoria.html` y `GET /super/api/pagos/auditoria` son exclusivos de `super_administrador`; el endpoint queda envuelto por `WithSuperAuditoria`, es de solo lectura y omite datos personales, payloads y secretos de pasarela. No concede acceso a roles empresariales.
