> Histórico. La descripción classic_form fue reemplazada al contrastar la ruta activa.
...
- Licencias y venta pública son flujos distintos aunque compartan webhooks. Referencia, importe, moneda, ambiente y firma deben coincidir con la orden local.
- La matriz de pagos define aceptación pendiente; no ejecuta cobros ni demuestra proveedor activo por disponer de claves.
...
14. Un estado `APPROVED` es monotono: callbacks tardios con estado pendiente, rechazado o error no pueden degradarlo ni volver a ejecutar la activacion.
15. Epayco solo puede aprobar con firma valida y evidencia que coincida con la referencia, el monto calculado por servidor, moneda COP y ambiente de la orden local.
16. Wompi exige correspondencia de ambiente entre `public_key`, `integrity_key` y el modo configurado; los eventos conservan validacion dinamica de `signature.properties` y secreto de eventos.
17. Las URL de retorno y confirmacion se obtienen de `gmail.confirm_base_url` o del dominio canonico; nunca se derivan de `Host`, `Origin` o `Referer` del cliente.
...
- los helpers de pagos deben fallar de forma segura si falta el esquema; la reparacion se hace exclusivamente mediante migraciones versionadas, nunca con DDL dentro de la peticion.
...
[contrato_matriz_pagos_reales.md](../../gobernanza_tecnica/contratos/contrato_matriz_pagos_reales.md), [main.go](../../../backend/main.go).
