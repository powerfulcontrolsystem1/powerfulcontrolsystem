...
El script Linux `deploy/scripts/vps-backup-operacion.sh` separa
`BACKUP_SCOPE=database|system|vps`. El alcance VPS incluye únicamente volúmenes
de contenedores activos con prefijos PCS/Nextcloud/OnlyOffice y excluye los
volúmenes físicos de motores de base de datos tomados en caliente. PostgreSQL y,
cuando aplica, MariaDB de Nextcloud se respaldan mediante dumps lógicos.
...
### Réplica fuera del VPS

`deploy/scripts/vps-external-backup.sh` crea el alcance indicado por
`BACKUP_SCOPE`, lo transfiere con `rclone` o S3 y verifica la copia remota. La
retención local predeterminada es de dos días. La retención larga se mantiene
fuera del servidor para evitar que una pérdida total o el propio crecimiento de
snapshots deje a PCS sin recuperación.

`rclone` es una dependencia operativa autorizada para interoperar con los
proveedores más usados sin introducir un SDK por proveedor en Go. La alternativa
en Go puro implicaría mantener autenticación OAuth, reanudación, checksums y
particularidades de cada API, aumentando superficie de seguridad y mantenimiento.
La dependencia afecta solo los scripts de backup y la integración del panel; no
cambia `go.mod`. Los tokens y el archivo `rclone.conf` son privados del host.
