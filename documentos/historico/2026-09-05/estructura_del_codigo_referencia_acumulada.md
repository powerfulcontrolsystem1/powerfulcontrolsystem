...
## Auditoria de pagos Super (2026-09-05)

- `backend/main.go` registra `GET /super/api/pagos/auditoria` con `WithSuperAuditoria`.
- `backend/handlers/payment_audit.go` valida filtros y expone solo metadatos operativos sin cache.
- `backend/db/payment_audit.go` combina los ledgers existentes de ePayco, Wompi e idempotencia mediante consultas parametrizadas de solo lectura.
- `web/super/pagos_auditoria.html` y `web/js/super_pagos_auditoria.js` muestran el reporte responsive desde el menu Comercial y licencias.
