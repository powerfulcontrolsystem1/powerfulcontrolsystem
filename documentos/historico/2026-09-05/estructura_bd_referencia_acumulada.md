...
`NUMERIC(18,2)`, moneda y origen (`manual`, `codigo_barras` o `ia_factura`). No
  ...
  La migracion `20260905-001-vida-reports-reminders-v1` agrega
  `empresa_vida_notificacion_configuracion` y `empresa_vida_notificaciones`.
  Ambas se aislan por `empresa_id + usuario_id`; el historial tiene unicidad por
  suscripcion, fecha de renovacion y canal para impedir avisos duplicados. No
  enlaza contabilidad, compras, caja, inventario ni cuentas empresariales.
...
- `pcs_superadministrador.licencias`: el catalogo base comercial queda normalizado en ocho planes globales con `tipo_id=0` y `pais_codigo='GLOBAL'`: prueba gratis 15 dias, prueba de 1 dia COP 5000, COP 60000, COP 110000, COP 200000, COP 600000 anual, COP 1100000 anual y COP 2200000 anual.
- La auditoria Super de pagos consulta `pagos_epayco`, `pagos_wompi`, `payment_checkout_idempotencia` y `payment_post_effect_idempotencia` sin crear una copia ni una tabla nueva. No selecciona `raw_payload`, llaves/hash de idempotencia, respuestas cacheadas ni errores remotos.
...
- `empresa_tarifas_por_minutos_configuracion.margen_desactivacion_habilitado` y `margen_desactivacion_minutos` almacenan por empresa la politica `Tiempo para cancelar el servicio`; vencido el limite, el backend rechaza cancelar o reiniciar una estacion abierta y exige pagarla.
- Las comisiones de servicio se persisten en `empresa_comisiones_servicio_movimientos` y generan evento contable `comision_servicio_causada` como costo laboral fiscal. La propina se mantiene en `empresa_propinas_movimientos` sin integrarse al total fiscal de la venta.
...
- 2026-05-13/2026-07-05: `licencias.max_cajas_simultaneas` fue historico; desde 2026-07-05 no aplica como regla comercial. Las licencias solo limitan documentos/ventas emitidas. `carritos_compras`, `empresa_ventas_estacion_metricas` y `empresa_finanzas_movimientos` siguen enlazando operaciones con `cierre_caja_id`, `caja_codigo`, `caja_turno` y `caja_sucursal_id` para cierres separados por caja.
...
## 2026-09-05 — Cuota empresarial compartida y rol de respaldo

- `empresa_api_rate_limits` reside en la base Super y conserva una fila por `(empresa_id, scope)`. La ventana por minuto y el contador se actualizan mediante UPSERT atómico para coordinar varias réplicas.
- La migración aditiva `20260905-002-empresa-rate-limit-v1` pertenece al catálogo Super y se aplica únicamente mediante `pcs-migrate`.
- `pcs-migrate` provisiona el rol indicado por `PCS_BACKUP_DB_USER` como login de solo lectura para ambas bases, sin capacidades administrativas ni DDL. Su secreto no se documenta ni se entrega al rol runtime de la API.
