...
- Nextcloud empresarial opera como stack separado en `deploy/nextcloud/docker-compose.yml`, con PostgreSQL y Redis. Se configura con un archivo de entorno privado y se publica por el proxy HTTPS del VPS; no se expone el puerto del contenedor a Internet directamente.
...
- Migra los volumenes `pcs_postgres_data`, `pcs_web_uploads`, `pcs_downloads`, `pcs_letsencrypt`, `pcs_certbot_www`, `pcs_onlyoffice_*`, `mailu_*`, `pcs_voice_*`, `pcs_rustdesk_data` y, cuando este activo, `pcs_nextcloud_*` del VPS principal.
