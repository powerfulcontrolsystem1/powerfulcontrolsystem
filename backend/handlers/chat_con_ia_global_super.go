...
func (c *SuperAIChatController) ConsultarHandler(w http.ResponseWriter, r *http.Request) {
	...
	respuesta, promptTokens, completionTokens, err := c.base.generateResponseWithSystemPrompt(model, payload.Pregunta, payload.Historial, buildSuperAISystemPrompt(contexto, superMeta, empresaRO, modoAsistente), empresaAISafetyIdentifier(adminEmail))
	if err != nil {
...
func (c *SuperAIChatController) ConsultarConAdjuntoHandler(w http.ResponseWriter, r *http.Request) {
	...
	superMeta := c.base.dbSuper != nil
	respuesta, promptTokens, completionTokens, err := c.base.generateResponseWithSystemPromptAndAttachment(model, preguntaFinal, historial, buildSuperAISystemPrompt(contexto, superMeta, empresaRO, modoAsistente), att, empresaAISafetyIdentifier(adminEmail))
	if err != nil {
