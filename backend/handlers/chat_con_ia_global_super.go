...
func (c *SuperAIChatController) ConsultarHandler(w http.ResponseWriter, r *http.Request) {
	...
	respuesta, promptTokens, completionTokens, err := c.base.generateResponseWithSystemPromptContext(r.Context(), model, payload.Pregunta, payload.Historial, buildSuperAISystemPrompt(contexto, superMeta, empresaRO, modoAsistente), empresaAISafetyIdentifier(adminEmailFromRequest(r)))
	if err != nil {
...
func (c *SuperAIChatController) ConsultarConAdjuntoHandler(w http.ResponseWriter, r *http.Request) {
	...
	superMeta := c.base.dbSuper != nil
	respuesta, promptTokens, completionTokens, err := c.base.generateResponseWithSystemPromptAndAttachmentContext(r.Context(), model, preguntaFinal, historial, buildSuperAISystemPrompt(contexto, superMeta, empresaRO, modoAsistente), att, empresaAISafetyIdentifier(adminEmailFromRequest(r)))
	if err != nil {
