...
func SuperVPS2Handler(_ *sql.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		cfg := loadSuperVPS2Config()
...
func loadSuperVPS2Config() superVPS2Config {
	cfg := superVPS2Config{
...
func buildSuperVPS2Status(cfg superVPS2Config) superVPS2Status {
	...
	out, err := runSuperVPS2SSH(cfg, superVPS2StatusCommand(), 12*time.Second)
	if err != nil {
		status.OK = false
		status.Errors = append(status.Errors, err.Error())
		return status
...
func superVPS2StatusCommand() string {
