...
func recordCarritoCancellationMetric(dbEmp *sql.DB, carrito, actualizado *dbpkg.CarritoCompra, estacionID int64, estacionCodigo, estacionNombre string, opts carritoCancellationMetricOptions) error {
	_, err := dbpkg.RecordCarritoStationMetric(dbEmp, dbpkg.CarritoStationMetricInput{
		EmpresaID: carrito.EmpresaID, CarritoID: carrito.ID, CarritoCodigo: carrito.Codigo, CarritoNombre: carrito.Nombre,
		EstacionID: estacionID, EstacionCodigo: estacionCodigo, EstacionNombre: estacionNombre,
		EventoOperacion: opts.eventoOperacion, OperacionCodigo: opts.operacionCodigo, MetodoPago: opts.metodoPago,
		Moneda: actualizado.Moneda, MontoTotal: actualizado.Total, MontoPagado: opts.montoPagado,
		MontoAnulado: opts.montoAnulado, DescuentoTotal: carrito.DescuentoTotal, DevolucionTotal: opts.devolucionTotal,
		ActivadoEn: actualizado.ActivadoEn, PagadoEn: actualizado.PagadoEn, ReferenciaOperacion: opts.referenciaOperacion,
		CierreCajaID: carrito.CierreCajaID, CajaCodigo: carrito.CajaCodigo, CajaTurno: carrito.CajaTurno, CajaSucursalID: carrito.CajaSucursalID,
		UsuarioCreador: opts.usuarioCreador, Observaciones: opts.observaciones,
	})
	return err
}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPut:
			...
			if action == "pagar_estacion" {
				...
				if errPago != nil {
					log.Printf("[carritos] pagar_estacion empresa_id=%d id=%d error: %v", empresaID, id, errPago)
					if errors.Is(errPago, dbpkg.ErrCarritoYaPagado) {
						...
						http.Error(w, errPago.Error(), http.StatusConflict)
						return
					}
					lower := strings.ToLower(strings.TrimSpace(errPago.Error()))
					if strings.Contains(lower, "metodo_pago") ||
						...
						http.Error(w, errPago.Error(), http.StatusBadRequest)
						return
					}
				...
				if comisionResultado.Aplicada && comisionResultado.MontoComision > 0 {
					registrarEventoContableNoBloqueante(dbEmp, r, "comisiones", dbpkg.EmpresaEventoContable{
						EmpresaID:       empresaID,
						Modulo:          "nomina",
						Evento:          "comision_servicio_causada",
						Entidad:         "comision_servicio",
						EntidadID:       id,
						DocumentoTipo:   "comision_laboral",
						DocumentoCodigo: strings.TrimSpace(carrito.Codigo),
						MontoTotal:      comisionResultado.MontoComision,
						Moneda:          strings.TrimSpace(carrito.Moneda),
						Origen:          "api_carritos_compra",
						Estado:          "causado",
						Observaciones:   "costo laboral fiscal por comision de servicio; no incrementa el total cobrado al cliente",
						...
						"tratamiento_fiscal":       "costo_laboral_comision",
						"incluida_total_documento": false,
					})
				}
				...
				modoDocumentoResuelto := "comprobante_pago"
				if documentIntent != nil && strings.TrimSpace(documentIntent.ResolvedMode) != "" {
					modoDocumentoResuelto = documentIntent.ResolvedMode
				}
				documentoVenta, errDocumentoVenta := registrarDocumentoVentaDesdeCarritoPagadoContext(r.Context(), dbEmp, dbSuper, carritoPagado, montoDocumento, usuarioOperacion, modoDocumentoResuelto)
				...
				writeJSON(w, http.StatusOK, map[string]interface{}{
					...
					"documentos_pendientes": errDocumentoVenta != nil,
					"factura_electronica":   carritoFacturaElectronicaResponse(documentoVenta),
					"modo_documento_venta":  modoDocumentoResuelto,
				})
				return
			}
			...
			if action == "anular_venta" || action == "anular_venta_total" {
				...
				estacionID, estacionCodigo, estacionNombre := dbpkg.ResolveCarritoStationIdentity(carritoActualizado)
				if errMetric := recordCarritoCancellationMetric(dbEmp, carrito, carritoActualizado, estacionID, estacionCodigo, estacionNombre, carritoCancellationMetricOptions{
					...
					metodoPago:      carrito.MetodoPago, montoAnulado: totalPagadoAnterior, devolucionTotal: devolucionTotalNueva,
					referenciaOperacion: motivo, usuarioCreador: usuarioOperacion,
			...
			if action == "anular_cierre_parcial" {
				...
				if errMetric := recordCarritoCancellationMetric(dbEmp, carrito, carritoActualizado, estacionID, estacionCodigo, estacionNombre, carritoCancellationMetricOptions{
					...
					metodoPago:      carritoActualizado.MetodoPago, montoPagado: totalPagadoNuevo,
					montoAnulado: montoAnulado, devolucionTotal: devolucionTotalNueva,
					referenciaOperacion: motivo, usuarioCreador: strings.TrimSpace(adminEmailFromRequest(r)),
...
func registrarDocumentoVentaDesdeCarritoPagadoContext(ctx context.Context, dbEmp, dbSuper *sql.DB, carrito *dbpkg.CarritoCompra, montoTotal float64, usuario, modoDocumentoVenta string) (map[string]interface{}, error) {
	...
	lockTipo := "comprobante_pago"
	lockCodigo := documentoCodigo
	if emitirFacturaEnEstaVenta {
		lockTipo = "factura_electronica"
		lockCodigo = buildVentaDocumentoCodigoFromBase(documentoCodigo, "factura_electronica")
	}
	lockedContext, releaseDocumentLock, documentLocked, lockErr := acquireFacturacionDocumentAdvisoryLock(ctx, dbEmp, carrito.EmpresaID, lockTipo, lockCodigo)
	if lockErr != nil {
		return nil, fmt.Errorf("reservar documento de venta: %w", lockErr)
	}
	if !documentLocked {
		return nil, fmt.Errorf("el documento de la venta ya esta siendo procesado")
	}
	defer releaseDocumentLock()
	ctx = lockedContext
	periodoContable := time.Now().Format("2006-01")
	...
	documentoYaExistia := false
	if existing, existingErr := dbpkg.GetEmpresaDocumentoFacturacionByCodigo(dbEmp, carrito.EmpresaID, "comprobante_pago", documentoCodigo); existingErr == nil && existing != nil {
		documentoYaExistia = true
	...
	if cfg.EnviarEmailVenta && !documentoYaExistia {
	...
	if emitirFacturaEnEstaVenta {
		...
		if facturaErr != nil {
			...
			return nil, fmt.Errorf("la venta fue pagada, pero su factura electronica requiere recuperacion: %w", facturaErr)
...
func validateCarritoPaymentPrerequisites(dbEmp *sql.DB, carrito *dbpkg.CarritoCompra, modoDocumentoVenta string) (*carritoBusinessPrerequisite, error) {
	...
	// Automatic frequency is advanced later under the payment transaction lock.
	// Validate every automatic-invoice sale up front so another simultaneous
	// cash register cannot turn a previously unchecked payment into an invoice.
	requiereFacturaElectronica := modoDocumentoVenta == "factura_electronica" || carritoAutoFacturaElectronicaActiva(cfg)
...
func carritoPaymentRequiresFacturaElectronica(dbEmp *sql.DB, carrito *dbpkg.CarritoCompra, modoDocumentoVenta string) (bool, error) {
	...
	return carritoAutoFacturaElectronicaActiva(cfg), nil
}
