...
func generateDynamicDocumentAIContent(ctx context.Context, dbEmp, dbSuper *sql.DB, payload DynamicDocumentRequest) (string, string, int64, int64, error) {
	...
	go func() {
		defer close(done)
		content, pt, ct, err = ctrl.generateResponseWithSystemPrompt(model, prompt, nil, system, empresaAISafetyIdentifier(fmt.Sprintf("empresa:%d:dynamic_document", payload.EmpresaID)))
	}()
	select {
	case <-ctx.Done():
