...
func createDomicilioOrderCarrito(dbConn *sql.DB, order EmpresaDomicilioOrder, usuario string) (int64, int64, error) {
	...
	if err := PayCarritoStationSession(dbConn, order.EmpresaID, carritoID, metodo, order.CodigoPedido, descuentoTipo, "", order.Descuento, 0, order.Total, 0, 0, 0, "", "", 0, strings.TrimSpace(usuario)); err != nil {
		return 0, 0, err
	}
