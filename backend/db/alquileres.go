...
func createOrSyncAlquilerContratoCarrito(dbConn *sql.DB, contrato EmpresaAlquilerContrato, marcarPagado bool, usuario string) (int64, int64, int64, int64, error) {
	...
	if err == nil && carritoExistente > 0 {
		...
		if marcarPagado && strings.TrimSpace(pagadoEn) == "" {
			_ = PayCarritoStationSession(dbConn, contrato.EmpresaID, carritoExistente, "transferencia_bancaria", contrato.Codigo, "", "", 0, 0, contrato.Total, 0, 0, 0, "", "", 0, strings.TrimSpace(usuario))
		}
	...
	if marcarPagado {
		if err := PayCarritoStationSession(dbConn, contrato.EmpresaID, carritoID, "transferencia_bancaria", contrato.Codigo, "", "", 0, 0, contrato.Total, 0, 0, 0, "", "", 0, strings.TrimSpace(usuario)); err != nil {
			return 0, 0, 0, 0, err
		}
	}
