...
func EnsureEmpresaGrafologiaSchema(dbConn *sql.DB) error {
	...
	columns := []struct {
		...
		{"empresa_grafologia_analisis", "cliente_id", "INTEGER DEFAULT 0"},
		{"empresa_grafologia_analisis", "cliente_nombre", "TEXT"},
		{"empresa_grafologia_analisis", "cliente_documento", "TEXT"},
		{"empresa_grafologia_analisis", "persona_descripcion", "TEXT"},
		{"empresa_grafologia_analisis", "persona_caracteristicas", "TEXT"},
...
func InsertEmpresaGrafologiaAnalisis(dbConn *sql.DB, item EmpresaGrafologiaAnalisis) (int64, error) {
	...
	err := queryRowSQLCompat(dbConn, `INSERT INTO empresa_grafologia_analisis (
		empresa_id, cliente_id, cliente_nombre, cliente_documento, persona_descripcion, persona_caracteristicas,
		titulo, archivo_nombre, imagen_url, imagen_mime, ocr_texto, ocr_motor,
		...
		item.EmpresaID, item.ClienteID, item.ClienteNombre, item.ClienteDocumento, item.PersonaDescripcion, item.PersonaCaracteristicas,
		item.Titulo, item.ArchivoNombre, item.ImagenURL, item.ImagenMime, item.OCRTexto, item.OCRMotor,
...
func ListEmpresaGrafologiaAnalisis(dbConn *sql.DB, empresaID int64, limit int) ([]EmpresaGrafologiaAnalisis, error) {
	...
	rows, err := querySQLCompat(dbConn, `SELECT id, empresa_id, COALESCE(cliente_id,0), COALESCE(cliente_nombre,''), COALESCE(cliente_documento,''), COALESCE(persona_descripcion,''), COALESCE(persona_caracteristicas,''), titulo, archivo_nombre, imagen_url, imagen_mime, ocr_motor,
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.EmpresaID, &item.ClienteID, &item.ClienteNombre, &item.ClienteDocumento, &item.PersonaDescripcion, &item.PersonaCaracteristicas, &item.Titulo, &item.ArchivoNombre, &item.ImagenURL, &item.ImagenMime, &item.OCRMotor, &item.Estado, &item.Resumen, &item.ConfianzaGlobal, &item.UsuarioCreador, &item.FechaCreacion, &item.FechaActualizacion); err != nil {
			return nil, err
		}
...
func GetEmpresaGrafologiaAnalisis(dbConn *sql.DB, empresaID, id int64) (EmpresaGrafologiaAnalisis, error) {
	var item EmpresaGrafologiaAnalisis
	err := queryRowSQLCompat(dbConn, `SELECT id, empresa_id, COALESCE(cliente_id,0), COALESCE(cliente_nombre,''), COALESCE(cliente_documento,''), COALESCE(persona_descripcion,''), COALESCE(persona_caracteristicas,''), titulo, archivo_nombre, imagen_url, imagen_mime, ocr_texto, ocr_motor,
		...
		&item.ID, &item.EmpresaID, &item.ClienteID, &item.ClienteNombre, &item.ClienteDocumento, &item.PersonaDescripcion, &item.PersonaCaracteristicas, &item.Titulo, &item.ArchivoNombre, &item.ImagenURL, &item.ImagenMime, &item.OCRTexto, &item.OCRMotor,
