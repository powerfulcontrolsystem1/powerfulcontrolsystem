...
class ApiClient {
  ApiClient(this._config, this._sessions, {http.Client? httpClient})
  ...
  Future<Map<String, dynamic>> _request(
    ...
    try {
      final streamed =
          await _http.send(request).timeout(_config.requestTimeout);
      ...
      if (response.statusCode == 401 &&
        ...
        if (refreshed != null) {
          return _request(method, path,
              query: query, body: body, headers: headers, allowRefresh: false);
        }
      }
      ...
      throw const ApiException('network_unavailable',
          'No fue posible conectarse con Powerful Control System.');
    }
  }

  Future<String?> _refreshAccessToken(String currentToken) {
    return _refreshInFlight ??= _performRefresh(currentToken)
  ...
  Future<String?> _performRefresh(String currentToken) async {
    try {
      ...
      await _sessions.saveToken(replacement);
      return replacement;
    } on Exception {
      return null;
    }
