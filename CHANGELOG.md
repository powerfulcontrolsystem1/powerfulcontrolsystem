- 2026-07-15: `Canalizacion de despliegue protegida` separa los recursos de staging (worker, almacenamiento privado y volumenes), impide que `rs` o `sync_to_vps` publiquen una rama de trabajo, evita transportar secretos Docker desde el equipo local y hace que un fallo del sincronizador llegue al orquestador con codigo distinto de cero. Se agregan contratos estaticos de staging/despliegue en CI y el workflow movil solo produce artefactos Android firmados cuando el entorno protegido aporta las credenciales de firma.
- 2026-07-15: `Worker, outbox y operacion reproducible` mueve los temporizadores de la API a `pcs-worker`, tipa pools PostgreSQL, bloquea DDL heredado en API/worker de produccion, agrega prioridad/version/heartbeat/recuperacion a cola y outbox, y exige la migracion requerida desde `/ready`. El worker dispone de healthcheck por heartbeat privado y la auditoria reconoce `scripts/rs.ps1` como ruta canonica. Se retira la plantilla escolar de catalogos y preconfiguraciones; el migrador desactiva su tipo historico sin borrar datos de empresas existentes. Se documentan despliegue inmutable y gates por modulo.
- 2026-07-15: `Preproduccion operacional` separa definitivamente el arranque productivo: `pcs-migrate` aplica el bootstrap historico bajo advisory lock PostgreSQL, API y worker ya no ejecutan DDL al iniciar, se agregan `/health` y `/ready`, y la cola durable incorpora expiracion, actor, correlacion e idempotencia hash. Los endpoints ERP generico y visitas publicas verifican una migracion existente en vez de crear tablas durante una solicitud.
