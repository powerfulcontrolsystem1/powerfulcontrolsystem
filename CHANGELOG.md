- 2026-06-08: `DIAN consumidor final y firma canonicalizada` ajusta el adquiriente consumidor final al ejemplo oficial DIAN (`AdditionalAccountID=2`, `TaxScheme ZY / No causa`) y calcula digest XAdES de documento, `KeyInfo`, `SignedProperties` y `SignedInfo` sobre XML canonicalizado; la factura diagnostica `SETP990000189` llego a DIAN pero fue rechazada por `ZE02`/`FAK61`, por lo que no se enviaron notas hasta tener `StatusCode=00`.
