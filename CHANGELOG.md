- 2026-07-28: `Migración reproducible desde PostgreSQL vacío` agrega las raíces mínimas `empresas`, `administradores` y `sesiones` antes del catálogo heredado cuando el rol exclusivo `migrate` habilita bootstrap explícito. Incluye un ensayo Docker efímero de base vacía y segunda pasada; API y worker continúan sin DDL.
