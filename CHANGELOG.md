- Super — **Página principal** (`/super/pagina_principal.html`): sincronización de tema en iframe (cookie/localStorage), fondo del `body` con variables del tema, título de cabecera legible en modo claro y estilos **`mp-card`** en tema claro para contraste (también mejora páginas super que reutilizan `mp-card`, p. ej. formatos de email).
