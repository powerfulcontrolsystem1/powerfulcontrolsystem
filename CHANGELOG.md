- 2026-08-31: `Raspberry / sensores de puertas para moteles` agrega un uso seleccionable por controlador, independiente de Domótica. Una placa puede barrer cuatro entradas GPIO 0–3 contra 1–16 salidas selectoras GPIO 4–19, con retardo empresarial de 10–5000 ms y hasta 64 canales asignables a habitaciones. El túnel autenticado crea y actualiza los dispositivos de `sensor_puertas`, conserva autoarranque/reconexión y registra alta, configuración, instalador, enrolamiento, asignación y desactivación de cada Raspberry, además de los cambios de estado de puertas. Incluye tutorial visual de conexión BCM/pin físico y seguridad de 3,3 V, migración `20260831-002-raspberry-door-sensor-v1`, pruebas Go y validación sintáctica del agente/JavaScript. Cambio local: sin migración aplicada, hardware real, `rs` ni despliegue.
- 2026-08-31: `Auditoria empresarial / permisos y evidencia forense` exige `auditoria:D` para `retener` y `purgar`, separa las exportaciones de vista de las forenses y entrega para estas últimas manifiesto SHA-256 y cadena de custodia por página filtrada. Sin despliegue.
- 2026-08-26: `Carrito POS / historial inmutable y corte de caja` deja de reconstruir ventas desde el estado actual del carrito reutilizable. Cada pago persiste en la misma transacción una operación única por cierre, caja/turno, componente en efectivo y snapshot de ítems en `empresa_ventas_estacion_metricas`; el outbox usa esa identidad por venta y el corte obtiene detalle/totales desde el historial, con dos consultas menos. Los rangos parciales ya no heredan el efectivo acumulado de un turno abierto antiguo. La migración versionada `20260826-002-cart-sale-history-v1` prepara instalaciones existentes; cambio local pendiente de ejecutar `pcs-migrate`, sin despliegue.
- 2026-08-26: `Carrito POS / venta real PCS` cierra una venta sola autenticada de una unidad QA, subtotal COP 100, IVA COP 19 y efectivo COP 119. El único `PUT pagar_estacion` respondió 200 en 562 ms, emitió el comprobante de pago 123, sumó COP 119 una vez a la caja `QA-FE`/cierre 29, conservó un solo movimiento de inventario y abrió el siguiente carrito vacío con pago bloqueado; sin errores de consola. El stock de la referencia quedó en -1 conforme a la política vigente que permite venta sin stock. El candidato continúa local y sin despliegue.
- 2026-08-25: `Carrito POS / carga sin repeticiones` agrupa el render inicial después de las lecturas paralelas, conserva una segunda pintura solo para ítems/abonos del carrito seleccionado y comparte `/me` y `estacion_prefs`. La prueba autenticada con Powerful Control System detectó y corrigió la selección prematura del código canónico: el candidato consulta primero la lista empresarial para reconocer el carrito directo legado, completó 15 lecturas API reales con estado 200 en una cascada de 1,179 s y resolvió la búsqueda de producto en 113 ms, sin errores de consola. La validación local aprobó escritorio/móvil, sintaxis, contratos, suite Go completa y `go vet`; no cambia seguridad, inventario ni pago y continúa sin despliegue.
- 2026-08-25: `Administrar empresa / carga inicial` paraleliza las lecturas independientes del shell, comparte temporalmente sesión y preferencias, reutiliza en el panel los datos ya obtenidos por el contenedor y evita inyectar ayuda en el iframe `about:blank`. Conserva permisos, licencias y `empresa_id`; agrega regresión local y no se despliega.
- 2026-08-25: `Productos, Servicios, Inventario y Compras` endurece recepción, existencias, costos, lotes y Kardex con transacciones, validación por `empresa_id` e idempotencia durable. La cola de impresión reclama con `FOR UPDATE SKIP LOCKED`, cierra por agente propietario y reintenta sin quedar agotada; Carrito comparte un solo resolver de impresora. Aprobaron suite Go completa, vet, sintaxis, auditoría sin rutas/IDs/funciones repetidas y revisión visual móvil/escritorio. Es candidato local para staging; no se declara producción hasta PostgreSQL/staging, carga IA real, agente/impresión física y despliegue autorizado.
- 2026-08-25: `Chat IA simple, multimodal y por usuario` repara el botón flotante que dejaba el drawer con `hidden`, simplifica el encabezado y el compositor, agrega historial agrupado por conversación y adjunta fotos/documentos. El historial normal pertenece a la cuenta autenticada dentro del tenant; `super_administrador`, `administrador_total` y `admin_empresa` pueden solicitar el historial de todos los usuarios de su empresa. Super Administrador define un único modelo global para chat y adjuntos; el valor inicial queda en `GPT-5.6 Terra` con razonamiento `medium` mediante Responses API. Se retira el interruptor de modo: el único Agente PCS queda siempre activo, mientras las herramientas continúan limitadas por rol, tenant y confirmación. No se despliega este cambio.
...
## 2026-08-23 - Catalogo publico simplificado

- La portada, las landings descriptivas y el editor super publican el corte de
  52 modulos activos y 13 plantillas empresariales.
- Se filtran configuraciones antiguas para evitar que reaparezcan tarjetas de
  modulos o plantillas retirados y se agregan pruebas de regresion.
- La documentacion vigente queda alineada con la consolidacion de
  Drogueria/Farmacia dentro de Inventario. Sin despliegue.
## 2026-08-25 - Pagos Colombia y hardware POS

- Se endurecen datafonos, Bre-B, Nequi/Wompi y bascula con validacion server-side, firma oficial de eventos, controles SSRF, estabilidad de peso y estados fail-closed.
- Los cobros automaticos de datafono permanecen bloqueados hasta homologacion contractual explicita del proveedor.

## 2026-08-26 - Idempotencia punta a punta

- Checkout Wompi/Epayco, API movil y acciones Rappi incorporan claves, huella de
  solicitud, replay durable y conflicto ante reutilizacion distinta.
- Activacion de licencia y correo posterior al pago usan barreras de maximo una
  vez; un resultado remoto ambiguo queda pendiente de conciliacion y no se
  repite automaticamente.
- Pago de carrito, efectivo, historial y outbox quedan atomicos; la contabilidad
  de venta pagada agrega unicidad por empresa/carrito.
- Offline, CxP, DIAN, venta publica, propuestas IA, outbox y jobs reciben claims,
  fingerprints o transiciones monotonas adicionales.
- Se agregan dos migraciones versionadas, pruebas y auditoria. Cambio local sin
  migrar, desplegar ni declarar GO de produccion.
- 2026-08-31: `Vida personal` agrega en Administrar empresa gastos cotidianos con foto/PDF privado, resumen mensual y suscripciones con avisos de renovacion o cancelacion. Cada fila y descarga exige empresa y cuenta autenticada, sin vista de otros usuarios ni integracion contable. Forma parte de los 53 modulos actuales y no depende del filtro comercial de licencias. Candidato local pendiente de `pcs-migrate`, prueba multiusuario/movil y despliegue autorizado.
- 2026-08-31: Vida incorpora historial personal de precios, codigo de barras por camara con alternativa manual y `IA: leer y registrar factura`. La captura IA valida un JSON acotado y crea gasto/productos en una transaccion sin tocar compras, inventario, caja ni contabilidad. Migracion aditiva `20260831-003`; validacion local, sin despliegue ni datos escritos en produccion.
