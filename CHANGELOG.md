- 2026-08-25: `Cierre local del acuse DIAN aceptado` sincroniza de forma idempotente la cola fiscal y el documento transaccional: un documento `pendiente_emision` solo pasa a `emitida` cuando existe aceptación y CUFE/CUDE oficial de 96 caracteres. El reintento de un acuse ya aceptado no vuelve a transmitir el XML y las familias comerciales no implementadas permanecen bloqueadas.
