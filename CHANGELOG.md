...
- 2026-06-25: `Modo rapido del carrito` agrega en Configuracion carrito el check `Activar modo rapido del carrito`, guardado como `modo_rapido_carrito`. El carrito aplica una vista compacta tipo texto/tabla, con menos decoracion visual, sin duplicar flujos de pago, inventario, descuentos ni facturacion. Se refuerza la carga del logo del carrito para ocultarse si la imagen no existe y se confirma PCS con logo mediano.
...
- 2026-06-25: `Carrito columna derecha y vista de impresion` reubica el logo del carrito como tarjeta propia en la columna derecha, encima de `Totales y detalles`, agrega un boton de carrito rapido junto al boton de pantalla completa y guarda por empresa la ultima opcion del select `Vista e impresion` en Buscar ventas y facturas.
