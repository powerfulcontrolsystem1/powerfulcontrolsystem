- 2026-09-06: el PR 231 completa sus contratos de CI: el ejemplo staging declara el rol PostgreSQL de respaldo sin secretos reales, la prueba Go distingue paquetes opcionales omitidos de errores y la vista Super de colas mueve CSS/JavaScript fuera del HTML para no aumentar bloqueos CSP. El análisis estático corrige cierres SQL y limita `pg_dump` a las bases autorizadas.
- 2026-09-06: `Facturacion multicaja, recuperacion pospago y contingencia Colombia` reserva numeracion legal por empresa/documento, serializa la frecuencia automatica entre cajas, congela la decision documental en el outbox del pago y permite al worker reconstruir documentos faltantes sin repetir cobro. Agrega historial tenant-scoped de autorizaciones de papel, incidentes, ledger y plazo operativo; la transcripcion DIAN tipo 03 permanece bloqueada hasta completar su generador y validacion especificos. PostgreSQL 16 desechable, Go completo, vet, 13 pruebas JS y preflight completo quedan verdes; candidato no desplegado.
...
- 2026-08-26: `Categorias y proveedores sin duplicados por mayusculas` agrega indices PostgreSQL por `(empresa_id, LOWER(BTRIM(nombre)))`. La migracion verifica ambos catalogos antes de aplicar cambios, falla cerrada ante colisiones historicas y conserva el mismo nombre en empresas distintas; los conflictos se publican como HTTP 409 seguro.
- 2026-08-26: `Login E2E reintentable` hace efectivo el segundo intento que ya declaraba el auditor visual: una navegacion o interaccion transitoria del login queda registrada, espera un segundo y repite el flujo completo antes de fallar. El error final conserva la causa tecnica sin incluir credenciales.
- 2026-08-26: `Bodegas ajenas o inexistentes como 404 seguro` unifica actualización, activación/desactivación y borrado para que `RowsAffected=0` no se convierta en 500. El resultado no distingue entre un ID inexistente y uno perteneciente a otra empresa.
- 2026-08-26: `Tendencia diaria de inventario compatible con PostgreSQL` normaliza el valor `DATE` devuelto por el driver antes de unirlo con la serie de días. El Kardex deja de mostrar una gráfica en cero cuando los movimientos y el balance por bodega sí contienen actividad; una fecha no reconocible falla cerrada.
- 2026-08-26: `Conflictos de unicidad legibles en el catalogo` clasifica como HTTP 409 los códigos/nombres/SKU/barras repetidos de categorías, productos, proveedores y servicios dentro de una empresa. La respuesta no filtra el error ni el nombre de la restricción PostgreSQL y los errores de infraestructura continúan como 500.
- 2026-08-26: `Carga IA de facturas repetidas sin reprocesamiento` resuelve dentro de la misma empresa la cadena auditada `duplicado_soporte_id`, reutiliza una extracción ya verificable y evita volver a invocar el proveedor IA. Si el original no existe, la cadena es cíclica o no contiene datos reutilizables, el formulario falla cerrado y no guarda la compra.
- 2026-08-26: `Productos e inventario como candidato productivo` corrige proveedores y servicios sin codigo, valida valores economicos/stock/impuestos/duracion, clasifica referencias hijas ajenas como 404 seguro, propaga errores tardios de SQL, moderniza lotes/seriales/reservas con selectores, elimina datos demo de Inventario y Compras avanzados, permite recepciones de compra multiitem atomicas e idempotentes y conserva facturas XML procesadas por GPT-5.5 como adjuntos privados bien formados y sin DTD. El cierre de CI, staging y evidencia visual corresponde al SHA final de PSI-000.
- 2026-08-26: `Reconciliacion DIAN exclusiva de acuses aceptados` agrega `action=reconciliar_aceptados_local`, protegida con permiso `A`, para completar estado y CUFE/CUDE locales sin atravesar ningun camino de despacho. Las colas pendientes, fallidas o enviadas sin acuse se omiten de forma explicita; la respuesta informa el modo seguro, los omitidos y que la transmision XML esta deshabilitada.
- 2026-08-25: `Cierre seguro integral de facturacion DIAN` bloquea toda emision libre antes de numerar, exige fuente inmutable para facturas desde venta, serializa y hace reanudable la nota credito total, y solo anula con CUDE/acuse oficiales coincidentes. Acciones fiscales sensibles requieren aprobacion, GET DIAN desconocidos fallan antes del CRUD, el correo de produccion espera aceptacion y la bandeja exige CUFE y fuente real para anular. La reconciliacion completa estados/CUFE/CUDE locales de acuses ya aceptados sin retransmitir; Nomina permanece en preflight sin XML ni transmision. Regresion DIAN, `go test ./...` y `go vet ./...` aprueban localmente.
- 2026-08-25: `Cierre local del acuse DIAN aceptado` sincroniza de forma idempotente la cola fiscal y el documento transaccional: un documento `pendiente_emision` solo pasa a `emitida` cuando existe aceptación y CUFE/CUDE oficial de 96 caracteres. El reintento de un acuse ya aceptado no vuelve a transmitir el XML, completa también el código faltante de una cola histórica sin alterar intentos/fecha/acuse y mantiene bloqueadas las familias comerciales no implementadas.
- 2026-08-25: `Reenvio DIAN manual de cola terminal` permite que la accion empresarial explicita reactive, bajo el bloqueo del documento, una cola `fallido_terminal` sin CUFE ni TrackId. El worker no revive terminales y toda evidencia fiscal existente bloquea el reenvio para evitar duplicados.
- 2026-08-25: `Auditoria DIAN sin TrackId` conserva la respuesta real de cualquier operación de envío que no entregue identificador mediante una clave interna no reconsultable. Los TrackId oficiales mantienen prioridad y el centro DIAN distingue la auditoría de una referencia válida para `GetStatusZip`.
- 2026-08-25: `Nota credito DIAN de anulacion total` deriva una fuente fiscal inmutable de la factura aceptada, conserva lineas/impuestos/partes/CUFE/fecha y genera CreditNote multilínea con CUDE y referencias oficiales. Solo la acción especializada queda habilitada; nota parcial/libre y nota debito siguen cerradas. XSD UBL 2.1, firma, preflight y regresiones aprueban localmente; el acuse real continúa pendiente.
- 2026-08-25: `Acuse DIAN sincronico y cierre visible de factura` conserva en el historial toda respuesta `SendBillSync` sin TrackId mediante una referencia `sync:<documento>` no consultable, evitando que un error local oculte el rechazo/aceptacion real. La fuente fiscal admite IVA exento al 0 % y lineas excluidas con el tratamiento UBL correspondiente; el carrito muestra el numero de factura y cualquier advertencia DIAN aunque el resultado venga anidado. Pruebas Go completas, vet y JavaScript aprueban localmente; `1PCS8` sigue pendiente de despliegue, reintento y verificacion visual, por lo que no se declara produccion cerrada.
- 2026-08-25: `RUT DIAN por empresa` agrega en Facturacion electronica un cargador separado del Formulario 1876. El PDF real de hasta 12 MB se procesa temporalmente con GPT-5.5, el backend normaliza NIT/DV, ubicacion DANE y responsabilidades, y la UI exige revisar, aplicar y guardar los datos sin almacenar el archivo ni activar/emitir documentos fiscales. La validacion productiva confirmo ademas que una extraccion parcial conserva los valores empresariales existentes cuando la IA no devuelve un campo opcional.
- 2026-08-25: `Venta directa compatible con carrito heredado` deja de filtrar exclusivamente el codigo canonico terminado en `-0` durante la carga enfocada. El reconciliador puede recuperar `VENTA-DIRECTA-<empresa>` y evita crear otra fila `Venta directa`, que antes chocaba con la unicidad empresarial y ocultaba toda la operacion. Incluye regresion frontend; la factura fiscal real permanece pendiente de confirmacion y acuse DIAN.
- 2026-08-25: `Release DIAN inmutable` fija también `upload-permissions` al digest exacto de API. La primera promoción a staging detectó que el auxiliar intentaba usar el tag local inexistente `pcs-backend:staging` y se detuvo sin cambiar producción; el contrato Compose y una regresión impiden volver a introducir ese fallback mutable.
- 2026-08-24: `Factura DIAN desde fuente fiscal real` captura un snapshot privado e inmutable por venta, genera UBL multilínea sin partes, direcciones, productos ni identificadores ficticios, agrega códigos DANE validados a emisor y clientes, pre valida la fuente antes del pago y rechaza ajustes globales no distribuidos. También cifra secretos DIAN por tenant/campo, cierra rutas libres de firma/envío y publica nota crédito/débito como bloqueadas hasta tener fuente de ajuste. La auditoría autenticada de PCS confirmó configuración productiva, firma y credenciales locales válidas, pero también que la versión publicada todavía ofrece emisión manual y presenta alcance de red HTTP como disponibilidad DIAN; el candidato corrige ambas señales y evita que la activación local complete por sí sola el progreso. Una reconsulta real detectó además que el binario publicado degrada el estado global por el rechazo de un TrackId histórico; el dato global se restauró y el candidato limita esa escritura al historial individual. El preflight profesional completo, Go, vet, JavaScript, PostgreSQL 17.11, XSD UBL 2.1 oficial y XMLDSig independiente aprueban localmente; Schematron oficial, clon del esquema vigente, CI remoto corregido, despliegue y acuse de una venta real siguen pendientes.
- 2026-08-21: `Familias DIAN sin falsa compatibilidad` limitó entonces la emisión real al adaptador de factura/nota crédito/nota débito. La revisión del 2026-08-24 detectó que las notas aún carecen de fuente de ajuste inmutable y las bloqueó; esta entrada se conserva como historial del candidato anterior.
- 2026-08-21: `Facturación electrónica Colombia por alcance específico` elimina valores DIAN asumidos y hashes locales presentados como CUFE, serializa consecutivos y envíos por documento, usa `NUMERIC(18,2)`, mueve reintentos automáticos al worker con bloqueo por empresa y consulta de TrackId sin regenerar XML. XML firmado, acuse y PDF quedan privados y se verifican con SHA-256 en cada lectura; el correo de producción exige aceptación DIAN y ambos adjuntos íntegros. Sin despliegue ni emisión real.
- 2026-08-14: `Plan 110: venta digital cancelable y fallos observables` propaga el contexto HTTP por catalogo, ordenes, Wompi y entrega de licencias, incluidos los requests al proveedor. Asociacion de archivos, rechazo de pago, sincronizacion y entrega dejan de ocultar errores. Deuda DB 548 a 533; descartes 766 a 762. No se ejecuto `rs` ni se creo PR.
- 2026-08-14: `Plan 110: soporte remoto completamente cancelable` propaga el contexto HTTP por uso, dispositivos, sesiones, limites, tokens de visualizacion y credenciales WebRTC, conserva `empresa_id` y hace observable el fallo al auditar intentos bloqueados. La deuda DB baja de 571 a 548 y los descartes de 767 a 766. No se ejecuto `rs` ni se creo PR.
- 2026-08-13: `Plan 110: alta de soporte remoto cancelable` propaga el contexto HTTP a lectura/guardado de configuracion y alta de dispositivos, incluyendo el limite por empresa; un fallo al cargar el plan ya bloquea el alta en vez de omitir el control. La deuda DB baja de 575 a 571. No se ejecuto `rs` ni se creo PR.
- 2026-08-13: `Plan 110: tablero financiero sin DDL runtime` reemplaza seis `Ensure*` del tablero por readiness fail-closed, agrega el contrato de clientes y propaga contexto a KPI y estados financieros; la deuda DB baja a 575. Tras agotarse C:, se limpiaron solo caches Go reconstruibles, se liberaron 21 GB y las pruebas se repitieron usando cache temporal en D:. No se ejecuto `rs` ni se creo PR.
- 2026-08-13: `Plan 110: cajas simultaneas cancelables` propaga el contexto HTTP por apertura, cupo de licencia, busqueda por usuario, ingresos/egresos atomicos, cierre/aprobacion y eliminacion de cajas, incluyendo carrito, venta offline, datáfono y Corte de caja. El rollback queda observable, `RowsAffected` deja de silenciarse y la deuda baja a 576/767. No se ejecuto `rs` ni se creo PR.
- 2026-08-13: `Plan 110: movimientos financieros cancelables` propaga `r.Context()` por configuracion, CRUD de ingresos/egresos, comprobantes y periodos contables; corrige errores ignorados de `RowsAffected`, reemplaza un `Ensure*` alcanzable desde HTTP por readiness y reduce la deuda DB sin contexto de 597 a 587. No se ejecuto `rs` ni se creo PR.
- 2026-08-13: `Plan 110: facturacion electronica cancelable y deuda accionable` incorpora variantes `Context` en configuracion fiscal, deteccion de jurisdiccion, reserva transaccional de consecutivos y reintentos; los handlers principales propagan el contexto HTTP, revisan `rows.Err()` y el auditor publica los 30 archivos con mayor deuda. La linea base DB sin contexto baja de 610 a 597. No se ejecuto `rs` ni se creo PR.
- 2026-08-13: `Plan 110: cero LastInsertId en producción` migra veinte altas/UPSERT de diez repositorios al contrato PostgreSQL `RETURNING id`, elimina fallbacks y errores ocultos, y agrega regresión global. La deuda DB sin contexto baja a 610 y los descartes a 768. No se ejecutó `rs` ni se creó PR.
- 2026-08-13: `Plan 110: gobierno administrativo cancelable` propaga contexto HTTP al impacto de desactivación empresarial y a la validación de tipo/rol de usuarios; la deuda DB sin contexto baja de 633 a 630. No se ejecutó `rs` ni se creó PR.
- 2026-08-13: `Plan 110: pago CxP cancelable` propaga el contexto HTTP por toda la transacción idempotente de pago a proveedores y sus helpers PostgreSQL; hace visibles los fallos de outbox/recarga y baja la deuda DB sin contexto de 640 a 633. No se ejecutó `rs` ni se creó PR.
- 2026-08-13: `Plan 110: PostgreSQL nativo y Calculadora cancelable` reemplaza `LastInsertId` por `RETURNING id` en reportes, Calculadora y proveedores; elimina la escritura implícita del GET de configuración, el fallback SQLite de facturación y dos fallos silenciados del paz y salvo. La deuda DB sin contexto baja a 640 y los descartes explícitos a 771. No se ejecutó `rs` ni se creó PR.
- 2026-08-13: `Plan 110: contexto SQL transversal` propaga cancelación HTTP por plan de cuentas, cartera, gestión documental, producción, logística y la cadena de conversión/reportes de ventas; la deuda DB sin contexto baja de 662 a 653. No se ejecutó `rs` ni se creó PR.
- 2026-08-13: `Plan 110: helpers DB canónicos` consolida verificadores de columnas/índices y paginación repetidos en once repositorios, reduce la duplicación exacta de 52 a 49 grupos y baja su línea base sin alterar SQL empresarial ni ejecutar `rs`.
- 2026-08-13: `Plan 110: cero duplicaciones exactas` centraliza utilidades de valores, entorno, DSN PostgreSQL por túnel y métodos de health en paquetes internos con biblioteca estándar; el auditor baja de 18 a 0 grupos y las suites Go/vet aprueban. No se ejecutó `rs`.
- 2026-08-13: `Plan 110: auditor SQL preciso` excluye `r.URL.Query()` de la métrica de base de datos, conserva las operaciones reales sin contexto y corrige la deuda de 1.841 a 689 con prueba de regresión. No se ejecutó `rs`.
- 2026-08-13: `Plan 110: instalador SSH Domótica sin errores ignorados` maneja deadline, cierre de socket y advisory lock; las pruebas enfocadas y gosec G104 aprueban con cero hallazgos. No se ejecutó `rs`.
- 2026-08-13: `Plan 110: reportes SQL cancelables` propaga el contexto HTTP a las 17 consultas y mutaciones de plantillas, programaciones, ejecuciones y consistencia; la deuda real baja de 689 a 672. No se ejecutó `rs`.
- 2026-08-13: `Plan 110: RRHH e inventario SQL cancelables` propaga el contexto HTTP en vacaciones y lotes/series, usa `RETURNING id` para la trazabilidad PostgreSQL y baja la deuda de 672 a 662. No se ejecutó `rs`.
...
- 2026-08-13: `Plan 110: scripts externos en 14 páginas` mueve el único script inline de catorce superficies a controladores externos equivalentes, reduce la deuda CSP a 1.269 bloqueos en 195 páginas y valida contenido exacto y sintaxis 14/14. No se ejecutó `rs`.
- 2026-08-13: `Plan 110: lote CSS de 23 páginas` externaliza los estilos del shell empresarial, CRM, WMS, MRP, ayuda, marca, auditoría, seguridad y configuración super; reduce la deuda CSP a 1.283 bloqueos en 209 páginas. Go, vet y preflight estricto aprobaron; la repetición visual desplegada continúa pendiente y no se ejecutó `rs`.
- 2026-08-13: `Plan 110: cuatro módulos sin inline` externaliza los estilos de Tesorería, Proveedores de Firma Digital, Soporte remoto y Tutorial de Domótica, reduce la línea base CSP a 1.306 bloqueos en 232 páginas y valida escritorio/móvil sin overflow ni errores de consola. No se ejecutó `rs`.
- 2026-08-12: `Plan 110: CI sin pasos indefinidos` limita `go test -race` a doce minutos con timeout interno de diez y la construcción de imágenes a veinte minutos, después de cancelar una corrida congelada sin convertirla en aprobación. No se ejecutó `rs`.
- 2026-08-12: `Plan 110: CSS compartido de configuración` externaliza el bloque visual duplicado de seis accesos empresariales, reduce la línea base CSP a 1.310 bloqueos en 236 páginas y valida las rutas en escritorio/móvil sin desbordes. No se ejecutó `rs`.
- 2026-08-12: `Plan 110: eventos inline en cero` elimina los 19 atributos `on*` restantes en nueve superficies, centraliza impresion/cierre/autoimpresion y respaldo de imagenes en un helper externo, y reduce la linea base CSP de 1.335 a 1.316 bloqueos. La suite Go y el preflight estricto aprobaron; no se ejecutó `rs`.
- 2026-08-12: `Plan 110: compuerta CSP decreciente` inventaría scripts/estilos/eventos inline por página y área, fija una línea base máxima de 1.335 bloqueos y falla CI ante regresiones. Grafología externaliza todo su CSS inline y reduce doce bloqueos, quedando lista para una futura CSP aplicada estricta. No se ejecutó `rs`.
...
- 2026-07-30: `Recuperación controlada del outbox CxP` agrega en el panel PostgreSQL del super administrador una vista previa obligatoriamente filtrada por empresa y limitada al topic de pagos CxP. La reactivación exige IDs únicos, razón y confirmación exacta, falla si alguna fila no continúa `dead`, conserva auditoría transaccional y no repite el pago: solo reanuda el procesamiento contable idempotente. El esquema nuevo pertenece a `pcs-migrate`; API y worker permanecen sin DDL.
- 2026-07-30: `Precisión monetaria visible en Finanzas` conserva el formato compacto para importes enteros y muestra dos decimales cuando existe unidad menor, evitando que un pago CxP real de `0,01` aparezca como `$ 0`. El candidato `f7214329` quedó validado visualmente en PCS y el mismo pago produjo outbox publicado, job completado, evento único y asiento balanceado.
- 2026-07-30: `Regresión empresarial ampliada del Plan 108` corrige el resumen de Créditos con cartera vacía, hace idempotente la siembra simultánea de Contabilidad Colombia, instala el sincronizador CSRF en 19 páginas empresariales mutantes, propaga el contexto de empresa en rutas explícitas del auditor y elimina el desborde móvil de Recordatorios de Cobranza. QA: `go test ./...`, `go vet` enfocado, contratos de módulos/roles/despliegue, 96 combinaciones autenticadas y 18/18 formatos imprimibles.
- 2026-07-28: `Rol PostgreSQL runtime sin DDL` separa el propietario usado por `pcs-migrate` del login de API/worker. El migrador rota la credencial y concede DML, secuencias y funciones, pero rechaza superusuario, CREATEDB, CREATEROLE, BYPASSRLS o CREATE sobre `public`.
- 2026-07-28: `Migración reproducible desde PostgreSQL vacío` agrega las raíces mínimas `empresas`, `administradores`, `sesiones`, `configuraciones` y `tipos_de_empresas` antes del catálogo heredado cuando el rol exclusivo `migrate` habilita bootstrap explícito. Incluye ensayos Docker efímeros de base vacía, upgrade lógico de staging y segunda pasada; API y worker continúan sin DDL.
- 2026-07-28: `Candidato P108 promovido por digest` cierra CI/race/Trivy/SBOM del commit `fce1655c`, publica API, migrador, worker y frontend en GHCR y despliega esas cuatro referencias exactas en staging con `--no-build`. Salud, listo, migración y regresiones autenticadas de carrito/correo quedaron verificadas sobre el artefacto inmutable.
- 2026-07-28: `Configuración operativa del carrito` alinea el SELECT PostgreSQL de reglas por rol con sus 18 destinos de Scan, incluyendo permisos de ingresos y egresos manuales. Corrige el HTTP 500 reproducido en staging para PCS sin cambiar el filtro `empresa_id`; el shell empresarial elimina además la advertencia por permiso fullscreen duplicado.
- 2026-07-28: `Candidato inmutable y acciones móviles super` incorpora el frontend al contrato obligatorio por digest, prepara construcción/Trivy/SBOM/publicación de las cuatro imágenes PCS y promoción de staging sin recompilar. Las acciones globales de páginas super dejan de cubrir controles móviles y permanecen en el flujo del documento.
- 2026-07-26: `Correos de registro por red privada Mailu` conecta exclusivamente backend y worker a `pcs_mailu_internal` y fija el SMTP interno en la IP privada configurada. Corrige los rechazos reales `554 5.7.1 Relay access denied` sin habilitar relay público. El autoregistro administrativo pendiente puede volver a solicitar confirmación y la pantalla ya no lo expulsa al login cuando el envío falla. Los mensajes conservan el logo real `img/Logo pcs 1.png` embebido y el activo público BIMI declara el perfil SVG Tiny PS requerido para el avatar de dominio.
- 2026-07-26: `Build Docker sin chown recursivo` asigna `pcs:pcs` durante cada `COPY` de las imágenes backend/worker/migrate y elimina el recorrido completo de `/app`, que se bloqueó durante más de 25 minutos en una construcción fría del VPS.
- 2026-07-26: `Firma DIAN heredada en volumen restringido` corrige el runbook para preparar únicamente la carpeta de facturación y los PEM de la empresa solicitada desde el host del VPS, validando el volumen real de uploads. La migración continúa como usuario no privilegiado y ya no depende de `chown` dentro del backend con `cap_drop: ALL`; el backend puede atravesar también el directorio padre heredado.
...
- 2026-07-11: `Seguridad pendiente de revision y despliegue` prepara la rama aislada `security/full-hardening` con OAuth state/PKCE, sesiones verificadas por hash, cifrado versionado, cabeceras HTTP y logging minimizado. No se publico; consultar `documentos/endurecimiento_seguridad_2026_07_11.md`.
- 2026-07-09: `Panel super: continuidad Docker real` corrige falsas prioridades de RustDesk, Backend, Nginx y PostgreSQL: el monitor valida los contenedores PCS y deja `systemd` como fallback; RustDesk deshabilitado no cuenta como incidente.
- 2026-07-09: `Retiro integral de juegos/emuladores y contexto documental` elimina frontend, runtime, assets, rutas, Docker, records y tabla exclusiva `super_juegos_records`; agrega contexto general obligatorio e indice especifico para agentes. Tambien corrige `rs` para aislar los `exit` de sus scripts internos y completar la sincronizacion, y hace que `sync_to_vps` borre rutas retiradas antes de extraer el paquete en VPS.
- 2026-07-09: `Gobernanza exclusiva Codex` retira `copilot-instructions.md` y consolida las instrucciones vigentes en `AGENTS.md`, `contexto_general_del_sistema.md` y `contexto_especifico_del_sistema.md`.
- 2026-07-09: `Facturas electronicas compactas y tutorial DIAN acordeon` mueve `Diagramas tecnicos` debajo de Configuracion en super administrador, compacta `Filtros de busqueda` de Facturas electronicas en una sola grilla, mejora la visualizacion imprimible con tablas de documento/totales, muestra como factura electronica las ventas que tienen factura asociada y convierte el tutorial DIAN en pasos desplegables con `Software propio` como unica modalidad PCS.
- 2026-07-09: `Panel super compacto, WhatsApp y ventas de licencias` saca `Panel` del grupo de menu del super administrador, elimina el grupo `Gobierno`, mueve `Auditoria global` a Plataforma, fuerza que el modulo abra en el panel, desplaza la estrella de favoritos para no chocar con el menu flotante, agrega tarjeta de consumo WhatsApp API, tarjeta de ventas de licencias de Powerful Control System con barras mensuales y compacta las tarjetas del panel sin reducir el tamano de letra.
- 2026-07-09: `Facturas electronicas filtros y anulacion por rol` compacta `Filtros de busqueda` en una sola tarjeta, corrige iconos gigantes/superpuestos, agrega selector de usuario/cajero con usuario actual detectado y oculta `Anular` cuando el rol efectivo no tiene permiso `facturacion:D`.
- 2026-07-09: `Facturas electronicas por cajero y anulacion` agrega columna/filtro de cajero en la bandeja documental, fuerza filtro automatico para rol cajero, recuerda la ultima vista de impresion por empresa, compacta filas/botones con iconos y permite anular ventas simples o facturas electronicas con la ruta correcta.
- 2026-07-08: `Facturas electronicas como bandeja unica` elimina del menu empresarial el acceso `Buscar ventas y facturas`, corrige el texto de `Facturas electronicas` sin Beta, retira cabeceras comerciales de la bandeja, agrega resumen por ventas/facturas/anuladas, centraliza exportacion por formato y desplaza Favoritos para que no choque con el menu flotante.
- 2026-06-26: `Carrito, corte de caja y ventas` retira el modo/boton de carrito rapido para dejar un unico flujo de venta directa, mueve el control a `Pantalla completa` con texto visible, refuerza `Efectivo recibido` y `Vuelto`, oculta los campos configurables del reporte de turno en `Corte de caja` y compacta los filtros de `Buscar ventas y facturas` en una franja de ancho completo.
- 2026-06-25: `Carrito logo, pantalla completa y vuelto` ajusta la vista de venta directa para mostrar el boton de pantalla completa como icono compacto en la esquina superior derecha de Buscar producto, refuerza el logo de empresa justo encima de `Totales y detalles` y convierte `Efectivo recibido` / `Vuelto` en celdas alineadas tipo tabla 2x2. Se actualiza el cache-buster CSS del carrito y se neutralizan reglas globales antiguas que ocultaban el boton de pantalla completa.
- 2026-06-25: `Logo del carrito desde configuracion avanzada` corrige la lectura de `/api/empresa/configuracion_avanzada` en el carrito para tomar `payload.configuracion` cuando el endpoint devuelve el wrapper normal. PCS vuelve a resolver `logo_url` y puede pintar el logo encima de `Totales y detalles`.
- 2026-06-25: `Modo rapido del carrito` fue una prueba operativa de vista compacta; queda supersedida por la limpieza del 2026-06-26 que conserva un unico carrito de venta directa y retira ese control.
- 2026-06-25: `Opciones de logo en identidad visual` agrega en Identidad visual los checks repetidos para mostrar logo en factura y en carrito, junto al selector de tamano pequeno/mediano/grande. Configuracion carrito guarda el mismo tamano en `logo_empresa_carrito_tamano`, el carrito aplica clases visuales por tamano y el historial de productos del carrito recupera el boton `Regresar al carrito`.
- 2026-06-25: `Subida unificada de logos y logo de carrito` deja en Identidad visual un unico boton visible `Subir Archivo` que abre el selector y sube automaticamente el archivo elegido; el logo de factura usa el mismo patron. En el carrito, el logo de empresa se mueve dentro del panel `Totales y detalles`, justo antes del titulo, manteniendo la regla de mostrarlo solo si la opcion esta activa y existe URL/logo disponible.
- 2026-06-25: `Identidad visual y recarga del panel empresarial` corrige la pagina `Configuracion > Identidad visual` para que cargue la seccion real con `empresa_id`, sin iframe anidado que perdia el contexto. Las subpaginas de Administrar empresa con `shell_admin=1` vuelven al contenedor principal al recargar con Control+F5, conservando menu y pagina activa. La empresa interna `Powerful Control System` mantiene por defecto `/img/Logo pcs 1.png` como logo de empresa/factura sin sobrescribir logos personalizados en `/uploads/`. QA: sintaxis JS, pruebas Go enfocadas y validacion visual local con Playwright.
- 2026-06-24: `Logo de empresa en carrito` mueve el control de logo fuera del carrito hacia `Configuracion carrito` con el check `Mostrar logo de la empresa en el carrito`. El carrito ya no muestra checks internos ni usa `localStorage`; solo pinta un logo encima de `Totales y detalles`, y solo si la empresa tiene logo real cargado (`logo_url`, `logo_empresa_url`, `logo_factura_url` o `logo`). QA: parseo JS embebido, `git diff --check` y validacion DOM con Chrome headless.
- 2026-06-24: `Carrito decide venta sola o factura electronica` agrega en Configuracion carrito el check `Preguntar venta sola o factura electronica al pagar`. Cuando esta activo, el boton Pagar abre una ventana para elegir `Venta sola` o `Venta con factura electronica`; el backend recibe `modo_documento_venta`, fuerza o salta la factura solo para esa venta y conserva el modo automatico/frecuencia cuando no se envia eleccion. QA: `go test ./handlers -run "^$" -count=1`, parseo JS embebido de carrito/configuracion/panel/productos con Node y `git diff --check`.
- 2026-06-24: `Panel, red social, radio e IA flotante` agrega en la tarjeta Noticias del panel empresarial el boton `Ocultar noticia`, que marca solo la noticia actual como leida por empresa/navegador y permite que una noticia nueva vuelva a mostrarse. La red social pasa a `Red Social Empresarial` con portada corporativa real, la emisora no se reactiva por estado viejo del navegador y el chat IA evita duplicar el boton flotante cuando una pagina autoinyecta el drawer.
- 2026-06-24: `Radio, carrito y corte de caja` agrega cierre completo de emisora arriba/abajo, devuelve la tarjeta de noticias del panel empresarial al tema activo, incorpora historial no agrupado de productos del carrito con endpoint protegido y pagina privada, ajusta botones/vuelto del carrito y reorganiza corte de caja con filtro superior, acciones en fila, reporte estilo papel y check para no cerrar sesion despues de imprimir.
- 2026-06-23: `Noticias privadas, QR Bre-B y botones de archivo` protege noticias para que solo se abran con sesion desde Administrar empresa, usa portada real y filtrado por pais, retira visualmente imagenes por noticia, compacta Buscar ventas y facturas, agrega check de QR Bre-B en el carrito y tematiza globalmente los botones nativos de seleccionar archivo con icono. La confirmacion automatica de pagos Bre-B queda condicionada a conectar webhook/API bancaria real.
- 2026-06-23: `Panel empresa, carrito y consola de recuperacion` compacta Favoritos y Email corporativo del panel empresarial, oculta errores tecnicos IMAP, deja Buzon/Chat a ancho completo, agrega checks para activar/desactivar Favoritos y Email, abre noticias desde pagina privada empresarial, ajusta el carrito con botones cuadrados, `Devolver`, Vuelto bajo Efectivo recibido y Total/Efectivo mas visibles. Agrega `scripts/consola_de_recuperacion.ps1` con botones para backup/restauracion preparada/sync/actualizar/rs y publica `web/img/bimi-pcs.svg` para BIMI; se documenta que Gmail requiere DNS BIMI/DMARC para mostrar el logo en el avatar.
- 2026-06-22: `Primer ingreso de usuarios por invitacion` corrige el alta de usuarios creados desde Administrar empresa: la pantalla valida antes de enviar que la contrasena y su confirmacion coincidan, el backend acepta alias compatibles de confirmacion (`password_confirm`, `password_confirmation`, `confirm_password`, etc.) y las mutaciones finales de `users` pasan por la capa compatible PostgreSQL (`execSQLCompat`) para que el guardado de contrasena/contrato no falle en VPS. Los campos historicos tipo texto (`email_confirmado_en`, `fecha_acepta_contrato`, `password_actualizada_en`) guardan `CURRENT_TIMESTAMP` casteado a texto para evitar el error PostgreSQL `CASE types text and timestamp with time zone cannot be matched`. Se conserva validacion por `empresa_id`, token de invitacion, correo y documento. QA: `node --check web/js/login_usuario.js`; `go test ./db -run "EmpresaUsuario|UsuariosEmpresa" -count=1`; `go test ./handlers -run "EmpresaUsuario|Cajero" -count=1`.
- 2026-06-22: `Portal publico y panel empresarial` integra en `index.html` paginas publicas modernas para Contacto, Privacidad y datos, Quienes somos y Sistema Hotel / Motel, accesibles desde el menu `Mas` sin salir del portal. La pagina de privacidad explica separacion de datos por empresa, responsabilidad de respaldo, descarga de informacion y eliminacion definitiva. En Administrar empresa, el chat interno del panel queda apagado por defecto y se activa desde Configuracion > Panel de inicio; el buzon queda centrado cuando el chat esta apagado, conserva historico de mensajes y tareas, se recupera la tarjeta de Email corporativo junto a Favoritos, y Noticias enlaza a la pagina publica de noticias con tipografia destacada.
- 2026-06-21: `Backup completo VPS PCS` agrega `scripts/crear_backup_vps.ps1`, una herramienta independiente con ventana de progreso para crear backups versionados en `D:\Backup vps PCS`. La copia incluye dump logico PostgreSQL, imagenes Docker locales PCS, volumenes Docker, archivos del proyecto sin `.env`/temporales, inventario operativo, SHA256, manifest local y script de restauracion para subir/preparar un VPS nuevo. Se ejecuto la primera copia real y quedo validada localmente con lectura del tar.
- 2026-06-21: `IA enfocada, capturas y radio simple` retira del chat normal los modos automaticos de documentos/reportes y los cierres con exportaciones no solicitadas; la voz queda apagada por defecto para empresas existentes. Los correos de arranque del servidor pasan a HTML corporativo con logo PCS visible. Ingresos, Egresos y Compras conectan sus botones IA al chat con el archivo cargado para revisar calidad de foto/documento y novedades antes de guardar. Radio online activa y reproduce con un solo boton `Reproducir`, minimizando al reproductor compacto. Super administrador retira el boton flotante de seleccionar empresa y deja la tarjeta del panel; la pagina de informacion para IA carga con texto UTF-8 y area amplia.
- 2026-06-20: `Panel de inicio configurable por empresa` agrega endpoint protegido `/api/empresa/panel_configuracion`, pagina `Configuracion empresarial > Panel de inicio` y soporte en el panel para ocultar `Noticias`, `Buzon de usuario` y `Chat de la empresa` por `empresa_id`. Las tres tarjetas quedan activas por defecto para empresas nuevas.
- 2026-06-19: `Prevención de errores de codificación` documenta la regla permanente de mantener UI, ayudas, correos, mensajes backend y documentación operativa en UTF-8 real, sin signos de reemplazo ni mojibake. `documentos/comandos_codex.md` agrega el barrido `rg` recomendado antes de cerrar cambios textuales o ejecutar `rs`.
- 2026-06-19: `Corrección de codificación y tildes` normaliza textos dañados por mojibake en pantallas y documentación clave. Se corrigen cadenas visibles como `Canales digitales y colaboración`, `Facturación electrónica`, `Venta pública`, `Catálogo público`, `Configuración`, textos de licencias, pagos y configuración guiada; también se corrigen mensajes Go de pagos/licencias y referencias documentales afectadas. QA: barrido `rg` sobre archivos web/backend/scripts/documentos principales sin remanentes visibles de codificación rota en pantallas activas intervenidas, chequeos sintácticos JS/Go y `git diff --check`.
- 2026-06-19: `Configuracion guiada no invasiva` corrige el panel empresarial para que el asistente inicial no vuelva a abrirse si la empresa ya quedo `pospuesta` o con `no_mostrar_mas`. El modal agrega check `No mostrar mas` y la pagina `Configuracion guiada` permite abrir el formulario directo o iniciar el chat IA/agente de configuracion manualmente desde Configuracion.
- 2026-06-19: `Menu empresarial con estado Beta` agrega una etiqueta `Beta` a los botones del menu principal de `administrar_empresa.html`, excepto Productos, Venta directa/Buscar ventas, Nomina, Reportes, Facturacion electronica y Configuracion. El cambio es visual y no modifica permisos, rutas ni licencias.
- 2026-06-19: `Empresas y licencias en super administrador` agrega al Centro de mando el conteo de empresas con licencia activa y sin licencia activa, con boton `Ver` hacia la lista filtrada. Nueva pagina `web/super/empresas.html` muestra empresas en modo solo lectura con filtros por nombre/NIT, licencia activa, sin activa, licencia de 15 dias y vencida. Backend: `/super/api/empresas_estado` entrega resumen consolidado con auditoria super.
- 2026-06-19: `Pagos de licencias y promo produccion` revisa el flujo de licencias con Epayco y Wompi: Epayco queda disponible en checkout publico y Wompi queda bloqueado de forma segura porque `wompi.integrity_key` no descifra con la `CONFIG_ENC_KEY` vigente; requiere reingresar la llave desde Super administrador para quedar operativo. Las tarjetas de licencias muestran precio anterior tachado para COP 60.000 (antes 70.000), COP 110.000 (antes 120.000) y COP 200.000 (antes 220.000). QA: pruebas enfocadas Go de pagos/licencias, chequeo sintactico JS, `rs` y validacion publica de checkout Epayco.
- 2026-06-19: `Logo corporativo en correos Mailu` agrega `email_corporativo.logo_url` con fallback a `/img/Logo pcs 1.png`, muestra campo y vista previa en Super administrador > Email corporativo Mailu, y envuelve los correos HTML del motor corporativo con el logo configurado. La prueba de envio Mailu ahora usa multipart HTML para validar visualmente la marca del remitente.
- 2026-06-18: `Configuracion interactiva de empresa e IA documental` hace que las empresas nuevas con preconfiguracion abran un asistente inicial por tipo de negocio al primer ingreso, guarda respuestas en `configuracion_guiada_interactiva`, agrega `agente_configuracion_de_empresa` al chat con selector de agente y acciones `UI_CLICK` confirmadas, y suma captura GPT-5.5 en Ingresos, Egresos, Compras y carta/lista de precios. Las extracciones descuentan cuota avanzada diaria por empresa y solo precargan formularios para revision humana.
- 2026-06-18: `Chat IA empresarial con agentes y limites` agrega catalogo de agentes al chat empresarial, selector de agente/modelo en Pedidos con IA, selector de agente en Centro IA empresarial y consumo de cuota diaria por empresa para agentes no generales. Consulta normal, adjuntos, streaming, Centro IA y pedidos registran auditoria `agente=...`; Pedidos con IA mantiene mutaciones solo por endpoint validado de ventas y Centro IA entrega borradores/recomendaciones sin modificar datos. Validado con Go, sintaxis JS y revision visual local escritorio/movil.
- 2026-06-18: `Correo corporativo Mailu como canal principal` convierte los envios automaticos a dominio propio: licencias/comercial usan `ventas@powerfulcontrolsystem.com` y alertas/usuarios/agente DIAN/facturacion/reportes/masivos usan `soporte@powerfulcontrolsystem.com`; Email corporativo agrega botones para provisionar ventas/soporte y probar envio real, retira Gmail SMTP del menu super, elimina envios directos Gmail en handlers y documenta diagnostico DNS/SPF/DMARC.
- 2026-06-18: `Agentes con cuotas por empresa` agrega limites diarios configurables para agentes por empresa (segundos, GPT-5.5 avanzado y GPT-5.4 mini ligero), registra uso en `empresa_agentes_uso_diario` y crea botones iniciales de `Agente internet` en Nomina e Impuestos que muestran propuesta actual vs sugerido sin aplicar cambios automaticamente.
- 2026-06-18: `Panel super reinicia indicadores y rs en scripts` mueve el uso canonico de `rs` a `scripts/rs.ps1`, retira el wrapper raiz, agrega en Centro de mando super botones para reiniciar metricas del panel y monitor de errores, y expone `/super/api/panel_control/reset` con auditoria super para limpiar solo `metrics` o `super_errores_sistema`.
- 2026-06-18: `Facturacion electronica, IA y barras de preparacion` retira el modulo OCR empresarial y su configuracion super, deja la captura de compras/gastos basada en IA GPT-5.5 con limites de super administrador, compacta la bandeja de facturas electronicas, agrega exportacion CSV/XLS de busquedas, deja un unico boton `Visualizar`, mejora la representacion carta/POS con tamano de letra ajustable y agrega anulacion fiscal por nota credito total. Nomina e Impuestos incorporan barra 0-100 de datos faltantes para preparar nomina electronica/nomina e impuestos. Validado con pruebas Go enfocadas, chequeo sintactico JS y pruebas visuales locales.
- 2026-06-18: `Agentes de mantenimiento IA y consumo OpenAI super` agrega en Centro de mando una tarjeta de consumo OpenAI de 30 dias, elimina barras internas de la pagina global de configuracion IA y crea `web/super/agentes_de_mantenimiento_qutomatico.html` para habilitar el agente diario DIAN, configurar hora/correo, ejecutar revision manual, registrar hallazgos y notificar por correo corporativo Mailu. El backend agrega `/super/api/agentes_mantenimiento`, worker diario, tablas `super_mantenimiento_agentes`/`super_mantenimiento_agente_hallazgos` y consumo mensual desde las tablas IA existentes.
- 2026-06-18: `DIAN produccion PCS aceptada y ayuda blindada` corrige la lectura operativa de `Regla: 90, Documento procesado anteriormente`: PCS ya no la marca como aceptacion automatica, sino como envio pendiente de consulta del acuse original. La configuracion de PCS (`empresa_id=12`) queda en produccion con prefijo `1PCS`, resolucion `18764111318575`, rango `1-100000` y Software ID asociado en DIAN. Prueba real: DIAN acepto por SOAP/WCF `SendBillSync` la factura `1PCS3`; el portal DIAN tambien muestra `1PCS3` y `1PCS2` como `Aprobado con notificacion`. La ayuda DIAN queda adaptada a PCS con control diario de consecutivos, lectura de `RUT01` y reglas para no reenviar documentos ya aprobados.
- 2026-06-16: `Codex navegador/VPS y carrito PCS` documenta el uso del navegador interno, pruebas visuales y `rs` en VPS para chats nuevos; ademas corrige el agregado por nombre en carrito PCS cuando buscar `menta` mostraba resultados pero el boton `Agregar` no agregaba.
...
- Apariencia frontend: limpieza estructural final en ayuda y vendedores.
	...
	- Descripción: `vendedores_licencias.html` elimina los `style=` residuales de layout y estados, mientras `ayuda.html` se normaliza por secciones completas de encabezados y párrafos auxiliares. El resultado es una herencia de tema más consistente y menos deuda visual repetida en páginas largas del panel super y de ayuda.
	- Verificación: diagnóstico del editor sin errores en los tres archivos tocados y búsqueda sin remanentes de `style="` en `web/super/vendedores_licencias.html` ni en `web/ayuda/ayuda.html`.

- Soporte remoto y RustDesk: simplificación operativa del módulo y control real del servicio desde super.
	...
	- Descripción: el frente de soporte remoto se simplifica para dejar solo la configuración esencial de RustDesk por empresa y el panel super pasa a tener control operativo del servicio del VPS con prueba real. Se agregan URLs macOS, se consolidan descargas oficiales para cliente y servidor y se elimina de las pantallas la lógica extensa de visor/sesiones como superficie principal. La UI super se unifica además en una sola vista `RustDesk`, donde conviven estado del servidor, acciones del VPS y configuración mínima por empresa.
	- Verificación: `get_errors` sin errores en las cuatro vistas HTML nuevas y `go test ./handlers -run 'Test(SuperSoporteRemotoHandlerConfigGetAndUpdate|PublicSoporteRemotoResolverAccesoExponeDescargasRustDesk|SuperServidoresProbeHandlerReturnsRustDeskStatus|SuperSoporteRemotoHandlerListsCompaniesAndCreatesSession)$' -count=1`.
...
- Portal publico: el header del home ahora comparte estilo con la landing y expone `Crear cuenta`.
	...
	- Descripción: `index.html` reemplaza el botón superior propio por el mismo estilo `btn dark` usado en `descripcion_de_los_sistemas.ht`, agrega un CTA `Crear cuenta` hacia `/registrar_nuevo_usuario_administrador.html` junto a `Iniciar sesión` y ajusta el responsive del header para que ambos botones se mantengan compactos y legibles en móvil.
	- Verificación: diagnóstico del editor sin errores en `web/index.html` y `web/estilos.css`.

- Apariencia frontend: segunda pasada de limpieza de estilos inline en módulos operativos.
	...
	- Descripción: se reemplazan estilos inline de color, gradiente, ocultación, espaciado y alineación por utilidades CSS y clases semánticas para reducir deuda visual y mejorar consistencia entre los seis temas del sistema. Parte del HTML generado desde JavaScript ahora usa clases y `data-*`, y los mensajes de error/éxito en runtime dejan de depender de hex fijos.
	- Verificación: diagnóstico del editor sin errores en los archivos principales tocados, búsqueda sin remanentes de `style="...color/background..."` en el frente intervenido, sin asignaciones `style.color = "#..."` en los JS afectados y corrida dirigida `go test ./handlers -run 'Test(SuperSoporteRemotoHandlerConfigGetAndUpdate|PublicSoporteRemotoResolverAccesoExponeDescargasRustDesk|SuperServidoresProbeHandlerReturnsRustDeskStatus)$' -count=1` satisfactoria.
...
- Permisos super: pruebas dirigidas migradas a PostgreSQL y middleware robustecido.
	...
	- Descripción: el bloque de pruebas de permisos del panel super ya no depende de motor legado retirado y ahora usa esquemas efímeros en PostgreSQL, alineados con el túnel local documentado del proyecto. En la misma iteración, `AuthMiddleware` deja de fallar cuando no hay conexión `dbSuper` en flujos públicos, evitando un `panic` en la validación de rutas públicas de licencias.
	- Verificación: `go test ./handlers -run 'TestSuperEndpointsPermisosPorRol|TestAdministradorPuedeEditarYEliminarEmpresaDesdeRutaSuperProtegida|TestNuevoAdminRegistradoPuedeCrearSuPrimeraEmpresaViaRutaSuperProtegida' -count=1` y `go test ./utils -run '^TestAuthMiddlewareAllowsPublicLicenciaPaymentRoutesWithoutSession$' -count=1`.
...
- Apariencia global: contraste y componentes alineados en los seis temas.
	...
	- Descripción: el sistema deja de depender de varios colores fijos que rompían el contraste en modo claro y pasa a usar variables de tema para paneles, tarjetas, módulos embebidos, estaciones especiales y estados informativos. Con esto, los seis modos de apariencia mantienen mejor legibilidad y coherencia visual.
	- Verificación: diagnóstico del editor sin errores en los archivos frontend tocados.

- Login administrativo: recuperación por enlace directo sin token manual.
	...
	- Descripción: el correo de recuperación administrativa ahora dirige al usuario a un enlace directo de restablecimiento y la pantalla de `login.html` ya no solicita ingresar token manual. El token sigue validándose en backend, pero queda encapsulado en el enlace recibido por correo.
	- Verificación: prueba dirigida del handler administrativo de recuperación/restablecimiento y diagnóstico del editor sin errores en archivos tocados.

- Correo de recuperación: el enlace largo se reemplaza por un botón `Cambiar contraseña`.
	...
	- Descripción: la versión HTML de los correos de recuperación ahora prioriza un botón `Cambiar contraseña` y deja la URL extensa solo como respaldo visible. El envío administrativo también se genera como `multipart/alternative` para servir texto plano y HTML en el mismo mensaje.
	- Verificación: `go test ./handlers -run '^TestAdminPasswordRecoveryTemplateRendersButton$' -count=1` y diagnóstico del editor sin errores en `backend/handlers/auth_admin_handlers.go` y `backend/handlers/super_email_templates.go`.
...
- Backups empresariales: se implementa la Fase 2 de exportar/importar configuracion por empresa.
	...
	- Descripción: el módulo de backups ahora exporta e importa configuración completa por empresa en JSON canónico, con restauración sobre PostgreSQL y trazabilidad del origen importado. En la misma iteración se limpian referencias operativas falsas a motor legado retirado del entorno y de comentarios de arranque.
	- Verificación: prueba dirigida del handler de configuración empresarial y diagnóstico del editor sin errores en los archivos tocados.
...
- PostgreSQL: gobernanza endurecida y limpieza de soporte residual motor legado retirado.
	...
	- Descripción: el repositorio formaliza PostgreSQL como único motor permitido en reglas de agentes e instrucciones del proyecto. También se elimina el fallback motor legado retirado en memoria del paquete `db`, el dialecto SQL por defecto deja de aceptar motor legado retirado y el esquema de horarios queda solamente en sintaxis PostgreSQL.

- Estaciones: la tarjeta `Notas` ahora soporta múltiples recordatorios persistentes con repetición automática local.
	...
	- Descripción: la estación especial `Notas` deja la lógica de una sola nota y pasa a manejar varias notas dentro de la misma tarjeta, con selección activa, temporizadores independientes, restauración del countdown tras recargar y repetición automática configurable por minutos. El valor por defecto de repetición entra a `estaciones_config`, mientras el runtime múltiple se persiste localmente por `empresa_id`.
	- Verificación: diagnóstico del editor sin errores en los archivos frontend tocados; revisión de QA y backend sin cambios contractuales obligatorios en servidor.
...
- Estaciones: se agrega la estación especial Notas y el orden configurable de estaciones especiales.
	- Archivos modificados: `web/administrar_empresa/configuracion_de_estaciones.html`, `web/administrar_empresa/estaciones.html`, `web/administrar_empresa/configuracion_carrito_de_compra_empresa.html`, `web/estilos.css`, `documentos/descripcion_de_modulos`, `documentos/matriz_roles_permisos_pos_multiempresa.md`, `documentos/diagramas/estructura_del_codigo.md`, `documentos/historial_de_cambios`, `CHANGELOG.md`.
	- Descripción: el módulo de estaciones ahora permite decidir si `Caja`, `YouTube` y la nueva estación especial `Notas` se cargan antes o después del listado normal. `Notas` añade una tarjeta operativa con texto editable, temporizador programable, guardado del texto base y alerta visual/sonora cuando vence el recordatorio.
	- Verificación: diagnóstico del editor sin errores en los archivos frontend tocados.
...
- Documentacion: se depura `Pendiente Notas` y queda como backlog vigente.
	...
	- Descripción: se elimina del archivo la mezcla de credenciales y notas obsoletas, se retiran tareas ya implementadas y se reorganiza lo faltante en un plan priorizado con fases de ejecución y criterio de cierre.
	- Verificación: revisión documental contra `documentos/descripcion_del_proyecto`, `documentos/diagramas/estructura_del_codigo.md` y el estado actual del repositorio.

- Login administrativo: se corrige la activacion del ojito para mostrar la contraseña.
	...
	- Descripción: la inicialización del toggle de visibilidad quedó insertada en el lugar incorrecto dentro de `showMsg`, así que el control podía aparecer sin activar el cambio real del input. Ahora `initPasswordVisibilityToggles()` se ejecuta en el flujo principal de carga de `login.js`.
	- Verificación: diagnóstico del editor sin errores en `web/js/login.js`.

- Super y portal público: el WhatsApp del botón flotante de la portada ahora es configurable.
	...
	- Descripción: se agrega una tarjeta en configuración avanzada para editar el número del botón flotante de WhatsApp de la portada pública. El valor queda persistido en configuración super y el `index.html` ahora lo consume desde `/api/public/pagina_principal` en vez de mantenerlo fijo en el marcado.
	- Verificación: `go test ./handlers -run "Test(PublicPaginaPrincipalHandlerExposesLandingFields|GmailConfigHandlerSaveWhatsAppContactNumber)$" -count=1`.

- Login administrativo: se agrega control visual para mostrar u ocultar la contraseña.
	- Archivos modificados: `web/login.html`, `web/js/login.js`, `documentos/historial_de_cambios`, `CHANGELOG.md`.
	- Descripción: el campo de contraseña de `/login.html` ahora incluye un botón tipo ojo que permite alternar la visibilidad del texto escrito, reutilizando el mismo patrón visual de contraseñas usado en otros formularios del portal.
	- Verificación: inspección estática de `web/login.html` y `web/js/login.js`.

- Portal público: la landing descriptiva elimina el hero superior y la tarjeta de accesos rápidos.
	...
	- Descripción: `/descripcion_de_los_sistemas.ht` ahora abre directamente en el contenido detallado de soluciones. Se retiran el bloque `Catalogo unificado` y toda la tarjeta de `Accesos rapidos`, manteniendo el soporte de hash y los CTA públicos existentes.
	- Verificación: inspección estática de `web/descripcion_de_los_sistemas.ht` y `web/estilos.css`.

- Portal público: la landing descriptiva ahora adopta la apariencia activa y un menú interno más profesional.
	- Archivos modificados: `web/descripcion_de_los_sistemas.ht`, `web/estilos.css`, `documentos/descripcion_de_modulos`, `documentos/matriz_roles_permisos_pos_multiempresa.md`, `documentos/historial_de_cambios`, `CHANGELOG.md`.
	- Descripción: la zona `Accesos rapidos` de `/descripcion_de_los_sistemas.ht` deja la grilla plana y pasa a un menú ejecutivo con numeración, subtítulo y estado activo según la sección visible. También se reemplazan fondos y colores fijos por variables del sistema de apariencia para que la landing siga correctamente el modo claro u oscuro del portal público.
	- Verificación: inspección estática de `web/descripcion_de_los_sistemas.ht` y `web/estilos.css`.
...
- PostgreSQL runtime: se corrige el patrón `LastInsertId` en módulos críticos y se reencamina `/api/empresa/proveedores` al CRUD coherente con compras e inventario.
	...
	- Descripción: las altas que antes respondían con `LastInsertId is not supported by this driver` ahora usan `insertSQLCompat` o `insertTxSQLCompat`, con lo cual PostgreSQL puede devolver `id` vía `RETURNING`. También se asegura el esquema de venta pública antes de guardar configuración, items u órdenes y se fuerza la ruta `/api/empresa/proveedores` a usar la tabla `proveedores`, que es la misma validada por productos y compras.
...
- Modelo de base de datos ERP (Módulo interdependiente de Compras / Proveedores).
        ...
        - Descripción: Se crearon las tablas operativas empresa_proveedores, empresa_ordenes_compra, empresa_ordenes_compra_items, y empresa_compras_recepciones para soportar el ciclo de abastecimiento. Se conectó EnsureEmpresasComprasSchema al bootstrap del servidor.
        - Verificación: Ejecución local exitosa de tests para esquema relacional.

- Eliminación de páginas huérfanas frontend al carecer de uso actual.
	...
	- Descripción: Se limpiaron del proyecto 8 archivos \.html\ obsoletos o huérfanos para reducir el tamaño del repositorio y evitar confusiones en los directorios de operación y super admin.
	- Verificación: No existen referencias de enrutamiento ni clics a estas páginas en los menús o vistas dinámicas.
...
- Chat y tareas: selección múltiple de usuarios, fotos y validación estricta de empresa.
...
- Panel empresa y chat/tareas: el módulo colaborativo se refuerza como dashboard principal y valida referencias empresariales antes de persistir.
	- Archivos modificados: `backend/handlers/chat_tareas.go`, `backend/handlers/chat_tareas_test.go`, `web/administrar_empresa/chat_y_tareas.html`, `web/estilos.css`, `documentos/descripcion_del_proyecto`, `documentos/diagramas/estructura_del_codigo.md`, `documentos/descripcion_de_modulos`, `documentos/matriz_roles_permisos_pos_multiempresa.md`, `documentos/historial_de_cambios`, `CHANGELOG.md`.
	- Descripción: `chat_y_tareas.html` ahora se comporta como home operativo del panel empresa con tarjetas resumen, acciones rápidas y estados vacíos guiados. En paralelo, `chat_tareas.go` valida que conversaciones y tareas referenciadas existan dentro de la empresa antes de crear participantes, mensajes, tareas, citas o notas de voz, y limpia archivos de adjuntos si falla la persistencia posterior para no dejar huérfanos.
...
- Autenticación administrativa: `login.html` vuelve a ofrecer `Recordar usuario` para el acceso por correo.
...
- Panel empresa: Chat y tareas pasa a ser la vista inicial y su calendario compartido queda en primer plano.
	...
	- Descripción: `administrar_empresa.html` ahora abre preferentemente `chat_y_tareas.html` al entrar y mueve ese acceso al primer lugar del menú. La página colaborativa sube el calendario mensual al inicio, refuerza la explicación de agenda compartida por empresa y amplía el peso visual del tablero para que la administradora registre reuniones y los demás usuarios autorizados las consulten desde su cuenta.
...
- Venta pública y pagos: el catálogo público deja de fallar con error interno sobre esquemas legacy.
...
- Panel empresa: reservas endurece sus queries y evita URLs infladas que podían terminar en error 414.
	- Archivos modificados: `web/administrar_empresa/reservas_hotel.html`, `documentos/historial_de_cambios`, `CHANGELOG.md`.
	- Descripción: la vista de reservas ahora resuelve `empresa_id` desde el contexto activo del panel, limita el texto de búsqueda antes de enviarlo por query string y centraliza la construcción de URLs del módulo para no propagar parámetros largos o innecesarios. Además, la tabla deja de incrustar el JSON completo de cada reserva en atributos HTML y usa el estado local para editar, reduciendo el tamaño del DOM y evitando crecimiento accidental de la navegación.
...
- Panel empresa: administrar productos deja de caer por la carga obligatoria de proveedores en la vista principal.
	...
	- Descripción: la vista `Administrar Productos` ya no arranca consultando proveedores de forma obligatoria cuando el modo visible es `productos`. El selector de proveedor principal pasa a cargarse de forma perezosa al abrir el formulario del producto, de modo que una falla o restricción del submódulo de compras/proveedores no tumba toda la pantalla principal de inventario.
	- Verificación: `get_errors` sin errores en `web/administrar_empresa/administrar_productos.html`.

- Panel empresa: se corrige la duplicación recursiva del submenú de productos y demás shells anidados.
...
- Frontend web: se endurece la adaptación automática a celular sin alterar la estructura de menús.
	...
	- Descripción: la hoja compartida ahora fuerza en móvil el apilado real de filas de formulario, toolbars y filtros, neutraliza anchos mínimos e inline widths problemáticos en controles, y mejora el comportamiento de tablas y bloques con scroll horizontal cuando hace falta. Además, las vistas super de base de datos, errores, seguridad VPS y soporte remoto reciben media queries puntuales para evitar desbordes y columnas rígidas en celular sin tocar los menús existentes.
...
- Portal público y selector de empresas: se corrigen CTA públicos y se oculta navegación global fuera del alcance super principal.
	...
	- Descripción: la landing descriptiva ya no reutiliza destinos privados como `/administrar_empresa.html` en el CTA `Probar Gratis`; cuando una tarjeta apunta a una ruta protegida, el flujo público redirige al registro de administrador. En el selector de empresas, el menú lateral ahora toma el perfil real desde `/me` y solo mantiene visibles `Administradores` y `Reportes globales` para cuentas super principales; la navegación sensible queda oculta por defecto hasta resolver la sesión.
...
- Estaciones: la tarjeta de YouTube deja de depender de búsquedas embebidas rotas y pasa a reproducir referencias válidas.
	...
	- Descripción: la estación especial de YouTube ya no intenta incrustar resultados de búsqueda que el proveedor externo no soporta. Ahora reproduce solo URLs o IDs válidos de video/playlist mediante `youtube-nocookie`, muestra la referencia configurada dentro de la tarjeta y, cuando el valor guardado es texto libre, deja un estado visible con fallback a `Abrir YouTube` fuera del sistema.
...
- Estaciones y carritos: se mejora el mensaje visible cuando falla la apertura del carrito por estación.
	- Archivos modificados: `web/administrar_empresa/carrito_de_compras.html`, `documentos/diagramas/estructura_del_codigo.md`, `documentos/descripcion_de_modulos`, `documentos/matriz_roles_permisos_pos_multiempresa.md`, `documentos/historial_de_cambios`, `CHANGELOG.md`.
	- Descripción: el carrito unificado ya no deja al usuario solo con `Error cargando carritos` al fallar la carga inicial. Ahora muestra un estado contextual con traducción segura del error, botón `Reintentar carga` y retorno explícito a `estaciones.html` cuando el flujo viene desde una estación, evitando exponer literales técnicos del backend como `unauthenticated` o `forbidden`.
	- Verificación: `get_errors` sin errores en `web/administrar_empresa/carrito_de_compras.html`; validación estética de apertura en `http://127.0.0.1:8080/administrar_empresa/carrito_de_compras.htmlempresa_id=6&estacion_id=1&estacion_nombre=Estacion%201&carrito_codigo=EST-6-1`.
...
- Estaciones y carritos: se corrige la carga del carrito por estación sobre PostgreSQL real.
	...
	- Descripción: el listado de `/api/empresa/carritos_compra` deja de depender de un `GROUP BY` frágil con cliente e items y ahora cuenta items desde un agregado previo por carrito. Además, `totales_pago` y `metricas_estacion` dejan de usar `ROUND(..., 2)` en SQL y redondean en Go, evitando fallos de compatibilidad entre motor legado retirado y PostgreSQL. Esto elimina el error visible `Error cargando carritos` al abrir una estación y estabiliza los totales del panel.
...
- IA: el proyecto migra a Google Gemini como único proveedor y retira Ollama/DeepSeek del flujo operativo.
	...
	- Descripción: se reemplaza la IA en VPS basada en Ollama por Google Gemini en el chat global super y el chat con IA por empresa. Configuración avanzada queda reducida a una sola API key cifrada, un interruptor global y un interruptor del proveedor `google`. El script de arranque ya no abre túnel a Ollama y el VPS queda sin `ollama.service` ni binario asociado.
...
- Carritos y estaciones: el sistema adopta un carrito unificado configurable por empresa y por estación.
	...
	- Descripción: desaparece la bifurcación de UI entre carrito de compras, venta simple y carrito compacto. Las estaciones abren siempre `carrito_de_compras.html` y la pantalla muestra u oculta bloques según configuración global por empresa y configuración individual por estación almacenadas en `estaciones_config`.
...
- Inventario y productos: las compras preventivas y por proveedor pasan a la sección `Compras` del submódulo.
	...
	- Descripción: la vista central del módulo ahora expone `view=compras` y concentra allí el plan de reposición por proveedor, el consolidado de compra y el borrador/ciclo de orden; la ruta `productos/compras.html` deja el placeholder y redirige a esa vista real.
...
- Chat IA super y empresarial: se autorrepara el esquema legacy `super_ai_*` y `empresa_ai_*` y se amplía el timeout de Ambis Local sobre el túnel VPS.
	...
	- Descripción: el chat super ya no falla al consultar modelo preferido o uso diario sobre instalaciones PostgreSQL heredadas; la capa DB repara tablas/columnas faltantes al vuelo y el cliente de Ollama ahora soporta tiempos de respuesta más largos de `codellama:7b` cuando la consulta viaja por el túnel local al VPS.
...
- IA super y empresarial: se agregan switches por proveedor para desactivar DeepSeek sin afectar Ambis Local.
	...
	- Descripción: configuración avanzada ahora puede habilitar o bloquear por separado `DeepSeek Chat` y `Ambis Local`; los chats empresarial y global super solo muestran proveedores activos y hacen fallback automático cuando el modelo preferido quedó apagado.
...
- Portal público: se elimina el arcade y solo queda el emulador N64 adaptado para móvil.
	...
	- Descripción: el proyecto retira todas las experiencias de juego y el emulador NES. El portal conserva solo el emulador N64 con acceso público y diseño móvil, y el menú global enlaza de forma directa a esa única experiencia.
...
- Selector de empresas: las tarjetas quedan alineadas a la izquierda y conservan un tamaño visual uniforme.
	...
	- Descripción: se elimina el centrado del grid en `seleccionar_empresa.html` y se fija una relación de aspecto común para que todas las tarjetas se vean del mismo tamaño dentro del selector.
...
- Portal público: el menú flotante deja de mostrar accesos rápidos de juegos y centraliza la navegación en la página del arcade.
	...
	- Descripción: se retira el manifiesto JSON que alimentaba el menú flotante y se deja un único enlace a `/Juegos/menu_juegos.html`, evitando duplicar la lista de juegos fuera del lobby principal.
...
- Gobernanza tecnica: se agrega checklist documental para QA y soporte.
	...
	- Descripción: se agrega una checklist operativa breve para QA y soporte sobre repositorio documental, firmas y exportes regulatorios, y se refleja en la documentación general del proyecto y de arquitectura que un exporte no sustituye la evidencia versionada o firmada cuando el flujo es sensible.
...
- Gobernanza tecnica: se endurece la reconciliacion documental y la evidencia regulatoria.
	...
	- Descripción: la gobernanza tecnica ahora reconcilia explícitamente repositorio documental, firmas, interoperabilidad fiscal/contable y exportes multiformato, y fija que un exporte regulatorio no sustituye la versión documental vigente ni la firma asociada cuando el flujo exige evidencia reforzada.
...
- Chat IA super y empresarial: se agrega aviso visible de servicio apagado y se explicita la prueba contra VPS.
	...
	- Descripción: cuando la IA global está desactivada, ambos chats muestran un mensaje visible, deshabilitan el formulario y evitan que el usuario siga intentando consultar. En configuración avanzada, el botón queda rotulado como `Probar IA contra VPS` para reflejar que la prueba es real y no solo de catálogo.
	- Verificación: diagnóstico del editor sin errores en los HTML modificados.
...
- Gobernanza tecnica: se documentan interoperabilidad documental y contingencias de integraciones externas.
	...
	- Archivos modificados: `documentos/gobernanza_tecnica/README.md`, `documentos/gobernanza_tecnica/plan_implementacion_gobernanza_tecnica.md`, `documentos/gobernanza_tecnica/contratos/README.md`, `documentos/gobernanza_tecnica/runbooks/README.md`, `documentos/descripcion_de_modulos`, `documentos/matriz_roles_permisos_pos_multiempresa.md`, `documentos/descripcion_de_archivos`, `documentos/historial_de_cambios`, `CHANGELOG.md`.
	- Descripción: la gobernanza tecnica ahora cubre formalmente la interoperabilidad entre compras, facturación, repositorio documental y conciliacion fiscal, y agrega un runbook específico para incidentes de conectores API e integraciones bancarias.
	- Verificación: `get_errors` ejecutado sobre la documentacion creada y modificada, sin errores.
...
- Estaciones: se agrega tarjeta especial `YouTube` con vista embebida y ampliacion.
	...
	- Descripción: el modulo de estaciones ahora permite activar una tarjeta especial `YouTube` desde `estaciones_config`, mostrar una vista embebida adaptable al tamaño de la tarjeta y abrirla en un overlay aproximado de `500 x 500` mediante un cuadrito de maximización `[]`.
...
- Gobernanza interna: se implementa un equipo base de cuatro agentes con direccion centralizada en `agente_go`.
	...
	- Descripción: `agente_go` queda formalizado como agente principal, seleccionado por defecto a nivel de gobernanza del repositorio y responsable de dirigir a backend/DB, frontend/UX y QA/operacion, integrando una sola salida técnica y documental.
...
- Configuracion empresarial: el submenú `Configuración` ahora puede ocultar y mostrar su menú lateral en celular.
	...
	- Descripción: la página `administrar_empresa/configuracion_menu.html` carga `menu.js`, adopta el wrapper `admin-sidebar-mobile-collapsible` y agrega el mismo botón final de `Ocultar menú` / `Mostrar menú` usado por otros shells administrativos, para que el submenú de configuración también sea plegable en móvil.
...
- Licencias Epayco: el correo de activación ahora se reintenta de forma idempotente después de aprobados posteriores si la licencia ya quedó activa pero la notificación todavía no quedó confirmada.
	...
	- Descripción: el flujo Epayco deja de depender de una activación “recién creada” para enviar el correo; además, ahora también recupera el `customer_email` cuando la validación lo devuelve anidado en `data`, evitando perder la notificación si el webhook aprobó primero o si el primer intento falló temporalmente.
...
- Gobernanza interna: se formaliza el protocolo de delegacion y la plantilla comun de trabajo del equipo de agentes.
	...
	- Descripción: `agente_go` ya no solo dirige al equipo; ahora también aplica una matriz exacta de delegación por tipo de tarea y una plantilla de ejecución compartida por módulo, mientras cada especialista queda priorizado por módulos críticos del sistema.
...
- Checkout de licencias: Epayco queda en tarjeta blanca, compacta y sin correo visible.
	...
	- Descripción: `pagar_licencia.html` elimina el bloque separado de formas de pago Epayco, oculta el campo de correo del panel y deja el checkout en una tarjeta blanca mas pequeña y centrada; `web/estilos.css` adapta el branding del panel para ese layout.
...
- Empresas super: nueva pagina editar_empresa con eliminacion total confirmada.
	...
	- Descripción: se añade `editar_empresa.html` para actualizar nombre y descripcion de la empresa seleccionada y se incorpora `action=eliminar_total` en `/super/api/empresas`, que purga datos relacionados por `empresa_id` en la base operativa y en la base super tras confirmar el nombre exacto.
...
- Selector de empresas: administradores no-super recuperan las lecturas iniciales.
	...
	- Descripción: se corrige el `403` que impedía abrir `seleccionar_empresa.html` a cuentas con rol `administrador`, habilitando solo `GET /super/api/empresas`, `GET /super/api/tipos_empresas` y `GET /super/api/licencias` para esa vista, sin devolver permisos de escritura ni acceso al resto del panel super.
...
- Checkout Epayco: retorno con referencia real y pantalla de pago exitoso.
	...
	- Descripción: el retorno web de Epayco ahora prioriza `x_ref_payco` y el estado real de la pasarela para que la validacion posterior sí pueda activar la licencia; cuando el backend confirma `APPROVED`, el usuario sale a una pantalla de pago exitoso y de allí vuelve a `seleccionar_empresa.html`.
...
- Venta pública por empresa: Wompi y Epayco con credenciales propias por empresa.
	...
	- Descripción: cada empresa puede activar o desactivar Wompi y Epayco con sus propias credenciales, administrar esas llaves desde `configuracion.html` y reutilizarlas en su tienda pública; además los webhooks de ambas pasarelas ya actualizan las órdenes de `empresa_venta_publica_ordenes`.
...
- Exportes operativos: descarga silenciosa sin sacar al usuario del módulo.
	...
	- Descripción: los exportes frecuentes de clientes, asistencia, backups, tarifas por día y soporte remoto dejan de reemplazar la vista actual. El archivo se descarga en segundo plano y el usuario permanece en el mismo módulo.
...
- Navegacion general: misma pestaña por defecto.
	...
	- Descripción: la navegación normal del sistema deja de abrir pestañas nuevas y reutiliza la misma ventana actual. Se mantienen como excepción solo el contrato, los términos legales de pasarela y los popups técnicos de impresión o vista previa documental.
	- Verificación: búsqueda final de `target="_blank"|window.open(` limitada a excepciones esperadas; diagnóstico del editor sin errores en los archivos modificados.
...
- Licencias super: valor 0 ya no se oculta en edición ni en listado.
	...
	- Descripción: el CRUD de licencias en panel super conserva `0` como valor valido visible en la tabla y en el formulario de edición, evitando que una licencia parezca vacía al reabrirla.
...
- Login administrativo: Google y correo quedan en una sola tarjeta visual.
	...
	- Descripción: el bloque de acceso por correo deja de renderizarse como un formulario en caja separado dentro de `login.html`. Google, correo, recuperación y reset ahora comparten el mismo contenedor visual principal.
...
- Reportes globales super: graficos y lectura ejecutiva.
	- Archivos modificados: `web/super/reportes_globales.html`, `web/js/super_reportes_globales.js`, `web/estilos.css`, `documentos/diagramas/estructura_del_codigo.md`, `documentos/descripcion_de_modulos`, `documentos/matriz_roles_permisos_pos_multiempresa.md`, `documentos/descripcion_de_archivos`, `documentos/historial_de_cambios`, `CHANGELOG.md`.
	- Descripción: la vista global del panel super ahora añade graficos comparativos y una lectura ejecutiva automática del consolidado de empresas seleccionadas, sin cambiar el modelo de permisos ni crear dependencias frontend externas.
...
- Checkout publico de licencias: Epayco redirige la misma pestaña al checkout.
	...
	- Descripción: el flujo de Epayco deja de depender de una pestaña emergente y ya no arranca el polling antes de que el usuario entre a la pasarela. `pagar_licencia.html` guarda la referencia pendiente, redirige la misma pestaña a Epayco y usa `/epayco/respuesta.html` para retomar la verificacion al volver.
...
- Home público: botones superiores más compactos y centrados en móvil.
	...
	- Descripción: los botones `Registrarse o iniciar sesión` e `Informacion de contacto` del `index.html` ahora comparten un ancho más pequeño, menor altura visual y en celular se muestran centrados dentro del header.
	- Verificación: diagnostico del editor sin errores en `web/estilos.css`.
...
- Super: modulo de seguridad VPS Linux con panel, CLI, cron y exportes multiformato.
	...
	- Verificación: `go test ./vpssecurity/... ./handlers ./tools/vps_security_scan -run "TestSecurityVPS|TestGenerateArtifacts|TestCompareDetects" -count=1`; diagnóstico del editor sin errores en los archivos Go/HTML/JS/SH modificados para este cambio.
...
- Login admin y configuración Gmail: se simplifica el hint visual y se habilita edición directa.
	...
	- Descripción: el login administrativo ya no muestra el bloque `Se recordará ... / Olvidar`, aunque conserva la logica de `Recordar cuenta`, y la seccion Gmail SMTP del panel super deja de bloquear el correo remitente y los demas campos cuando ya existe una configuracion guardada.
...
- Login Google: host canúnico en dominio raíz y estaciones con carga visible.
	...
	- Descripción: se corrige la inestabilidad del acceso administrativo tras registrar el dominio público, redirigiendo `www.powerfulcontrolsystem.com` al host canúnico `powerfulcontrolsystem.com` antes de procesar OAuth y alineando los defaults de `GOOGLE_REDIRECT_URL` al callback del dominio raíz. Además, la página de estaciones ahora muestra `Cargando estaciones...` mientras consulta configuración, carritos y sensores, con mensaje visible en caso de error.
	- Verificación: `go test ./utils -run "Test(CanonicalPublicHostMiddleware|LoggingMiddlewareSetsContextAndWritesLogs|JSONErrorMiddlewareWrapsNonJSONError)" -count=1`; `go test ./handlers -run "TestHandleGoogleLogin" -count=1`; diagnóstico del editor sin errores nuevos en los archivos tocados.
...
- Sync VPS: limpieza automatica de procesos huerfanos antes del restart remoto.
	...
	- Verificación: en VPS `powerfulcontrolsystem.service` quedó `active (running)` tras limpiar el listener huérfano; `curl -k -I https://powerfulcontrolsystem.com/auth/google/login` sigue devolviendo `302` con `redirect_uri=https://powerfulcontrolsystem.com/auth/google/callback`.
...
- Portal principal: título en una sola línea con subtítulo debajo en la misma columna.
	- Archivos modificados: `web/index.html`, `web/estilos.css`, `documentos/historial_de_cambios`, `CHANGELOG.md`.
	- Descripción: se agrega el contenedor `portal-intro-copy` para apilar verticalmente el encabezado del home, manteniendo `Sistema de Facturación Electrónica` en una sola fila y moviendo `Toma el control de tu negocio con Powerful Control System` justo debajo, centrado en el mismo bloque visual.
...
- Modulo de impresoras operativas por empresa.
	...
	- Descripción: se añade gestion de impresoras por `empresa_id` (predeterminada, estado activo/inactivo, asignacion por funcionalidad y por producto) y resolución de carrito/finanzas/reportes.
...
- OAuth Google VPS: prioridad de entorno sobre DB + soporte de `GOOGLE_REDIRECT_URL` en despliegue.
	...
	- Verificación: `go test ./handlers -run "TestHandleGoogleLoginRedirect" -count=1` y `go test ./ -count=1` en verde. Diagn?stico en VPS confirma que el bloqueo actual es de política OAuth en Google (`secure-response-handling` / `redirect_uri_mismatch`) y no de base de datos.
...
- Migración PostgreSQL (fase 3): avance inicial en autenticación y sesiones.
	...
	- Descripción: se incorpora capa de compatibilidad SQL motor legado retirado/PostgreSQL (rebindeo de placeholders y expresiones de fecha) y se aplica a funciones críticas del flujo de autenticación/sesiones (`UpsertUser`, `UpsertAdministrador`, `CreateSession`, `RevokeSessionByToken`, `GetSessionByToken`, `GetAdminByEmail`).
...
- Migración de datos a PostgreSQL en VPS: instalación, ejecución por etapas y validación inicial.
	...
	- Descripción: se instala PostgreSQL en VPS por SSH, se crean las bases `pcs_superadministrador` y `pcs_empresas`, y se inicia la migración desde motor legado retirado con `pgloader` en dos etapas (superadministrador y empresas), validando consistencia por conteo de tablas en cada base. Se formaliza además la regla operativa: base productiva en VPS con PostgreSQL y motor legado retirado local como legado de migración/contingencia.
	- Verificación: `VALIDACION_SUPER_OK` y `VALIDACION_EMPRESAS_OK` tras comparación motor legado retirado vs PostgreSQL por tabla.
...
- Unificación de bases motor legado retirado: solo dos archivos canúnicos del sistema.
	...
	- Descripción: se normaliza la resolución de rutas en runtime para que el backend use por defecto `backend/db/pcs_empresas` y `backend/db/pcs_superadministrador` aunque se ejecute desde otro directorio. Se depuran copias operativas duplicadas en raíz y en `backend/`, dejando únicamente dos archivos `.db` activos.
...
- Sync VPS: bootstrap automático para servidor nuevo y diagnóstico de OAuth.
	- Archivos modificados: `scripts/sync_to_vps.ps1`, `scripts/README_sync.md`.
	- Descripción: se añade bootstrap post-sync en modo sin WSL para instalar dependencias base (`ca-certificates`, `curl`, `motor_legado_retirado`), asegurar `backend/.env.local` y reportar estado de variables críticas (`GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `SERVER_PORT`, `CONFIG_ENC_KEY`) con salida `BOOTSTRAP_WARN/BOOTSTRAP_OK`. Se incorporan parámetros opcionales `-GoogleClientId` y `-GoogleClientSecret`.
...
## 2026-04-04
- Se registra nueva credencial Gemini cifrada en configuración avanzada (`ai.model.google.gemini_2_0_flash.api_key` en `pcs_superadministrador`).
...
### Added
- Implementación de controladores HTTP (Handlers) para el CRUD de Proveedores integrado en el nuevo módulo de compras y logística ERP.
...
- 2026-06-19: `Correo corporativo y panel super producción` embebe el logo real `/img/Logo pcs 1.png` como imagen inline `cid:` en correos corporativos Mailu para evitar que clientes muestren solo la inicial del remitente. Compacta Auditoría global, Auditoría super, Códigos de descuento y Licencias; mueve `Seleccionar empresa` al toolbar superior del panel super; retira la página `2FA super`; filtra Administradores a `administrador` y `super_administrador`; agrega códigos de descuento con generación automática, nombre, descripción, email, vencimiento y reenvío; y suma limpieza controlada de empresas sin licencia activa por mínimo 6 meses. `sync_to_vps.ps1` sube timeouts internos de health/restart a 900s.
- 2026-06-19: `Super selector, menu personas y carrito flexible` repara el boton `Seleccionar empresa` del panel super para navegar a `/seleccionar_empresa.html` desde una accion integrada no flotante y azul clara; elimina el scroll duplicado de fondo al abrir el chat IA; mueve Usuarios, Clientes y Login de usuarios al submenu `Usuarios, clientes y personas`; retira `Beta` de Estaciones/Puntos de venta, Corte de caja, Compras, Centro financiero y contable y Clientes; deja las ventas sin stock habilitadas por defecto para todas las empresas; y mantiene el carrito con un estilo unico basado en la tarjeta `Detalle del pago`.
- 2026-06-19: `Configuracion guiada una sola vez` corrige el panel empresarial para que el check y el boton `No volver a mostrar` oculten inmediatamente el asistente inicial, sin exigir guardar el formulario. La decision se persiste localmente al instante y se envia al backend como `no_mostrar_mas`; el asistente solo vuelve a abrirse desde Configuracion guiada.
...
- 2026-07-06: `Panel super VPS2` agrega la pagina `web/super/vps2.html` y el endpoint auditado `/super/api/vps2` para consultar estado del VPS2 por SSH, ver CPU/carga/temperatura, memoria, disco, Docker, servicios y contenedores Nextcloud, y ejecutar acciones cerradas con confirmacion: reiniciar Nextcloud, reiniciar VPS2 o apagarlo. Las credenciales se leen solo desde configuracion local privada o variables de entorno. `sync_to_vps.ps1` excluye archivos volatiles de Nextcloud Sync/Windows para estabilizar el TAR de `rs`.
- 2026-07-06: `VPS2 operacion y sincronizacion` agrega `scripts/sync_to_vps2.ps1` para validar SSH, actualizar el repo remoto por Git, reiniciar Docker si hay compose, dejar el VPS2 en `multi-user.target` y asegurar Nextcloud con `restart unless-stopped`. Se documenta `documentos/vps2_operacion.md` con host key verificada, configuracion local privada `PcsVps2*` y evidencia de que `nextcloud-app`, `nextcloud-redis` y `nextcloud-db` quedaron activos. Las claves no se versionan.
- 2026-07-07: `Integracion Rappi por empresa` agrega pagina `web/administrar_empresa/rappi.html`, API protegida `/api/empresa/rappi`, webhook publico `/api/public/rappi/webhook`, tablas `empresa_rappi_configuracion` y `empresa_rappi_ordenes`, con credenciales por referencia y aislamiento por `empresa_id`.
# 2026-07-09 - RustDesk multiempresa y monitor Docker
...
# 2026-08-25 - reCAPTCHA operativo y respuestas seguras de autenticación

- La política CSP permite los orígenes exactos de Google necesarios para
