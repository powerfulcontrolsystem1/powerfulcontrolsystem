- 2026-05-11: portada index alineada a modulos reales. `web/index.html` y los defaults de `/api/public/pagina_principal` actualizan tarjetas publicas con nucleo unico y modulos reales; los 20 verticales nuevos siguen en catalogo, pero solo los 10 V1 masiva quedan como tarjetas operativas de `Probar gratis`, mientras los 10 diferidos se resumen sin publicarse como oferta operativa. `web/js/nuevos_verticales_catalogo.js` agrega decision, ranking, metadata de plantilla, permisos, flujo de venta y reportes. No hay endpoints, tablas, permisos ni dependencias nuevas.
- 2026-05-11: aseguramiento comercial de verticales v1. `POST /super/api/verticales_nuevos/catalogo?action=asegurar_v1_licencias` asegura tipos de empresa, preconfiguraciones y cuatro planes recomendados para los 10 verticales priorizados; la pantalla `Verticales v1` agrega boton `Asegurar v1` y refresca el semaforo. No hay tablas, rutas nuevas ni dependencias.
- 2026-05-11: semaforo listo para venta en verticales v1. `web/super/verticales_produccion_masiva.html` cruza catalogo de verticales, preconfiguraciones y licencias activas para marcar `Listo venta` solo cuando el vertical es v1 masiva, tiene metadata completa, preconfiguracion activa con `integracion_vertical` y licencia activa que incluye el modulo. No hay cambios de esquema ni dependencias.
- 2026-05-11: acciones de gobierno para verticales v1. La vista super `Verticales v1` ahora enlaza cada vertical con `Tipos de empresa`, `Preconfiguraciones` y `Licencias`; esas paginas aceptan `q`, `vertical` o `modulo` para abrir filtradas desde la matriz comercial. No se agregan endpoints, tablas ni dependencias.
- 2026-05-11: gobierno super de verticales v1 masiva. Se agrega `web/super/verticales_produccion_masiva.html` y acceso `Verticales v1` en el panel super para auditar ranking, 10 priorizados, 10 diferidos, metadata de plantilla, permisos, flujo de venta, reportes y exportacion CSV usando `/super/api/verticales_nuevos/catalogo`. No hay endpoints, esquemas ni dependencias nuevas.
- 2026-05-11: preconfiguraciones conectadas a la matriz vertical extendida. `config_json` de tipos de empresa ahora puede incluir `integracion_vertical`; los catalogos de verticales nuevos publican `integracion_preconfig`, decision, prioridad y bandera `produccion_masiva`. Se priorizan 10 verticales para v1 masiva y 10 quedan diferidos con plan documentado en `documentos/plan_verticales_produccion_masiva_2026-05-11.md`. No hay cambios de esquema ni dependencias nuevas.
